...
```
                    ...
                    │  → 创建 .opsx/changes/<name>/     │
                    │  → 生成 .opsx.yaml (schema元数据) │
                    └──────────────────┬───────────────────┘
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
            ...
            │  │  → 写入 .opsx/changes/<name>/     │         │
            │  │  → 标记工件 status = done            │         │
            │  └─────────────────┬───────────────────┘         │
            │                    │                              │
            │                    ▼                              │
            │  ┌─────────────────────────────────────┐         │
            │  │ 4e. opsx status --change --json  │         │
            │  │  → 重新加载工件状态                  │         │
            │  └─────────────────┬───────────────────┘         │
            │                    │                              │
            │      ┌─────────────┴─────────────┐                │
...
| 条件 | 必须? | 说明 |
|------|-------|------|
| `applyRequires` 全部工件 `done` | ✅ 是 | `opsx status --json` 返回 `isComplete` 或所有必备工件完成 |
...
```
                    ...
                    │  → 否: 继续                          │
                    │                                      │
                    │  mv .opsx/changes/<name>          │
                    │     .opsx/changes/archive/        │
                    │     YYYY-MM-DD-<name>/               │
                    │                                      │
