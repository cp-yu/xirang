...
```
opsx profile set core|extended
opsx profile install <workflow>
opsx profile uninstall <workflow>
opsx profile list
opsx profile show
opsx config set delivery skills|commands|both
opsx config get delivery
opsx config list
```
...
**After (simplified):**
```
opsx config profile          # interactive picker (delivery + workflows)
opsx config profile core     # preset shortcut
opsx config profile extended # preset shortcut
```
...
- **Project** = repo-level config (`.opsx/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `opsx init`
2. `opsx init` applies current profile to project
...
- One `opsx/` per repo, always at root
...
├── services/
│   └── billing/
│       └── .opsx/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── .opsx/specs/checkout/spec.md
│   ├── ios/
│   │   └── .opsx/specs/checkout/spec.md
│   └── android/
│       └── .opsx/specs/checkout/spec.md
└── opsx/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `opsx/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `opsx/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `opsx/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `opsx/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
- `.opsx/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `.opsx/specs/_shared/checkout-contract/spec.md`
- Or `.opsx/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single OPSX root (one `opsx/` directory) |
...
2. `.opsx/specs/opsx-conventions/spec.md` for normative spec conventions
3. `.opsx/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules
...
- **Each repo keeps its own canonical `opsx/`** — no change to the fundamental storage model.
...
```yaml
# billing-service/.opsx/changes/add-3ds/.opsx.yaml
schema: spec-driven
created: 2026-04-12
initiative: add-3ds
links:
...
```yaml
# web-client/.opsx/specs/checkout/spec.md frontmatter
references:
...
|---------|----------|------------|
| **Spec organization** | Nested specs inside one `opsx/` (Model B) | Each repo has its own `opsx/` |
...
|----------|--------|-------|
| Profile UX | Decided | `opsx config profile` with presets |
