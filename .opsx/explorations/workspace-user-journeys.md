...
- a repo root with `opsx/`
- a monorepo root with one `opsx/`
...
During `/opsx:propose` or `/opsx:explore`, OPSX should ask something like:
...
- .opsx/specs/contracts/checkout
- .opsx/specs/shared/checkout
...
```text
Created .opsx/changes/add-dark-mode/
Using scope: repo
...
1. create a single monorepo-local change under `.opsx/changes/add-invoice-filtering/`
...
```text
platform/.opsx/changes/add-3ds-checkout/
```
...
```bash
opsx workspace create add-3ds --at ~/work/opsx-workspaces/add-3ds
```
...
- `contracts/.opsx/changes/add-3ds-contract/`
- `billing-service/.opsx/changes/add-3ds-billing/`
- `web-client/.opsx/changes/add-3ds-web/`
- `ios-client/.opsx/changes/add-3ds-ios/`
...
2. runs something like `opsx workspace doctor` or `opsx workspace sync`
...
```text
Related references:
- github.com/Fission-AI/contracts: .opsx/specs/checkout/spec.md
- github.com/Fission-AI/web-client: .opsx/specs/checkout/web/spec.md
```
...
1. each repo independently runs `opsx init`
...
- `opsx workspace doctor` or equivalent relinks roots
...
3. Should OPSX support nested `opsx/` roots, or keep one root and model scopes separately?
