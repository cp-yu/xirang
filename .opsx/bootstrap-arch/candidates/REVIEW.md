...
Candidates live at `.opsx/bootstrap-arch/candidates/` (outside formal workspace).
...
- v1 adaptations: `opsx { languageVersion '1' }`, nested under `project_root`, stable `elementId` (from historical `capabilityId`), **no** `capabilityId`/`metadata.specs`/`belongs_to`
...
- elementId: `project.root/domain.opsx_element`
  - `cap.opsx.atomic-write` — Atomic Write [deprecated]
  - `cap.opsx.bootstrap` — Bootstrap [deprecated]
  - `cap.opsx.bootstrap-backfill-specs` — Bootstrap Backfill Specs [deprecated]
  - `cap.opsx.bootstrap-refresh` — Bootstrap Refresh [deprecated]
  - `cap.opsx.delta-merge` — Delta Merge [deprecated]
  - `cap.opsx.read-transparent` — Read Transparent [deprecated]
  - `cap.opsx.referential-integrity` — Referential Integrity [deprecated]
  - `cap.opsx.semantic-relations` — Semantic Relations [deprecated]
  - `cap.opsx.yaml-operations` — Yaml Operations [deprecated]
...
- elementId: `project.root/domain.validation`
  ...
  - `cap.validation.opsx` — Opsx Element [deprecated]
...
- `opsx-semantic-relations`: `project.root/domain.architecture/cap.architecture.semantic-model` → `cap.opsx.semantic-relations`
...
- `opsx-framework-identity`: ambiguous:cap.ai.workflow-templates,cap.framework.identity
...
- `opsx-build` currently `project.root/domain.architecture/cap.architecture.bootstrap`
...
- Moved `.opsx/bootstrap/` → `.opsx/bootstrap-history/2026-07-23T09-27-23-000Z/`
- Also archived `project.opsx*.yaml.backup` into that stamp’s `yaml-backups/`

## Validation

- Assemble candidates into temp `.opsx/architecture` and run `node bin/opsx.js arch validate`
- Do not use global npm `opsx` for v1 profile
