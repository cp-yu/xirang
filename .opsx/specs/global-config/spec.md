...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/opsx`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/opsx` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\opsx`
...
全局配置 SHALL 存储用户级别的 OPSX 设置，不包含 profile、workflows 或 delivery 字段。

#### Scenario: 配置文件结构

- **WHEN** 读取全局配置
- **THEN** 配置文件 SHALL 位于 `~/.config/opsx/config.json`
