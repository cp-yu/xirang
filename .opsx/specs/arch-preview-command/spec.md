# arch-preview-command Specification
...
`opsx arch preview` SHALL 启动 likec4 预览服务器，在浏览器中可视化架构。

#### Scenario: 启动预览服务器

- **WHEN** 运行 `opsx arch preview`
- **THEN** SHALL 调用 `npx likec4 start .opsx/architecture/`
...
- **WHEN** 运行 `opsx arch preview --port 8080`
