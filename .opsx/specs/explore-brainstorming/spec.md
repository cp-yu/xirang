...
- **AND** commands、paths、artifact names、schema keys 和 OPSX tokens SHALL 保持 canonical
...
- **THEN** SHALL 使用 `opsx-propose` 逻辑 workflow 名称路由
...
- **WHEN** agent 读取 `.opsx/references/opsx-explore-supperpowers-style.md`
...
- **AND** SHALL 明确用户审查通过 `Design Summary` 后才能路由到 `opsx-propose`
...
- **AND** SHALL 包含指向 `.opsx/references/opsx-explore-supperpowers-style.md` 的 Required References 引用，且 SHALL 使用"必须阅读"级措辞
...
`opsx-explore` SHALL 在 todo 工具可用时，把 Superpowers explore 阶段记录为 checklist，并在阶段完成时更新 checklist。该 checklist SHALL 覆盖 context exploration、visual companion decision、one-question clarification、options comparison、section-by-section approval、Design Summary self-review 和 propose handoff。

#### Scenario: 主 instructions 要求 todo 跟踪

- **WHEN** 系统生成 `opsx-explore` skill 内容
