...
archive 操作 SHALL 在校验通过后将 change 目录移动到 archive 目录，并输出 git handoff 提醒。archive 不执行任何 spec 或 OPSX 写入操作。同步合并由 `opsx sync` 独立完成。
...
- **WHEN** executing `opsx archive change-name`
...
- **WHEN** executing `opsx archive change-name --no-validate`
