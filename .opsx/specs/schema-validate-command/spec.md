...
## Purpose
Define `opsx schema validate` behavior for validating schema syntax, structure, templates, and dependency graphs.
## Requirements
### Requirement: Built-in Schema validation

`opsx schema validate [name]` SHALL 校验一个或全部内置 Schema 的 YAML、Zod structure、`FileDefinition`、templates、artifact dependency DAG 与 bootstrap file references。命令 SHALL 保留 JSON 和 verbose output。

#### Scenario: 校验一个内置 Schema
- **WHEN** 用户执行 `opsx schema validate spec-driven --json`
...
- **WHEN** 用户执行 `opsx schema validate` 不带 name
