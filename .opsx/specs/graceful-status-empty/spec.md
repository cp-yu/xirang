...
- **WHEN** user runs `opsx status` without `--change` and no change directories exist under `.opsx/changes/`
- **THEN** the CLI prints `No active changes. Create one with: opsx new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `opsx status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `opsx status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `opsx status --change non-existent`
