...
`opsx sync` SHALL 复用 `change-sync` 契约执行同步。在同步执行之前，系统 SHALL 检查 verify gate（通过 `checkFreshness` 和 `checkArchiveCompatibility`），除非用户传入 `--no-verify`。Sync SHALL NOT generate or write scenario operation labels into change-local specs.
...
- **WHEN** `opsx sync my-change` prepares delta specs for writing
- **THEN** sync SHALL NOT run `opsx scenario-labels my-change --write`
