...
## Purpose
Define `opsx change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `opsx change show update-error --json`
...
- **WHEN** executing `opsx change list`
- **THEN** scan the .opsx/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `opsx change show update-error --requirements-only`
...
- **WHEN** executing `opsx change validate update-error`
...
- **WHEN** executing `opsx list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'opsx list' is deprecated. Use 'opsx change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `opsx list --all`
...
- **WHEN** executing `opsx change show` without arguments
...
- **WHEN** executing `opsx change validate` without arguments
- **THEN** display an interactive list of available changes
