...
- **AND** `specs` definition SHALL NOT 将 `opsx scenario-labels <name> --write` 声明为 validation command
...
- **WHEN** Agent 执行 `opsx instructions <artifact> --change <name> --json`
...
#### Scenario: Generic Specs guidance 不编排 scenario labels
...
- **AND** MUST NOT 直接要求执行 `opsx scenario-labels "<change>" --preview` 或 `--write`
...
- **AND** SHALL 将 `.opsx/specs/**/*.md` 标记为 workflow-managed durable behavior source
...
`opsx/project.opsx.yaml` 与 `opsx/project.opsx.relations.yaml` SHALL 被定义为共同组成 durable architecture source 的 workflow-managed formal bundle。前者 SHALL 定义 project metadata 与非 relation nodes，后者 SHALL 定义完整 canonical semantic relation 集；两者 MUST NOT 被定义为 change log、code map 或 bootstrap 私有状态。
