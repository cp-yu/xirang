...
系统 SHALL 提供只读命令 `opsx arch plan-remove <element-id-or-fqn>`，分析严格非级联删除一个 element 所需显式处理的 containment、semantic relationships、Element Contracts 与其他语义引用。

#### Scenario: 分析 formal element
- **WHEN** 用户对 Formal Architecture 中存在的 element 运行 `opsx arch plan-remove payment.authorize`
...
- **WHEN** 用户运行 `opsx arch plan-remove payment.authorize --change refactor-payment --json`
...
- **WHEN** command 在 Windows、macOS 或 Linux 定位 `.opsx/changes/<name>` 与 Specs
