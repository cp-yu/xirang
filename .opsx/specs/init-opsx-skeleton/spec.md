...
`opsx init` SHALL 在首次初始化（非 extend 模式）时仅在目标不存在时创建两个 OPSX v2 骨架：`opsx/project.opsx.yaml` 与 `opsx/project.opsx.relations.yaml`。两个文件 SHALL 包含 `schema_version: 2`；系统 MUST NOT 创建 `project.opsx.code-map.yaml`。
...
- **GIVEN** `opsx/` 已存在且包含用户编辑的 OPSX 文件
- **WHEN** 运行 `opsx init`
...
`opsx init` 成功提示 SHALL 在 getting started 区块之后、链接之前，显式引导用户运行 `/opsx:bootstrap` 完成架构映射。
