...
✗ Verify gate failed — <简述失败原因>
  ...
  建议操作:
    opsx verify phase1 <change-name>  # 重新验证
    opsx <command> <change-name> --no-verify  # 跳过门禁 (风险自负)
```

其中 `<command>` 由调用上下文决定 — sync 场景为 `opsx sync`，archive 场景为 `opsx archive`。如果 `archiveCompatibility` 指示不兼容，输出中 SHALL 包含对应的 `blockReason`。非阻塞的 Git HEAD informational 数据 SHALL NOT 作为失败段落输出。
...
- **THEN** 建议操作中 SHALL 使用 `opsx archive` 而非 `opsx sync`
