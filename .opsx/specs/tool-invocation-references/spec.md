---
element: cap.ai.tool-invocation-references
---
...
- **AND** guidance 需要引用另一个已注册 workflow surface
- **THEN** 渲染结果 SHALL 使用 `/<skillDirName>` 形式（如 `/opsx-propose`）
- **AND** `skillDirName` SHALL 来自共享 workflow surface manifest 的显式值
...
- **AND** guidance 需要引用另一个已注册 workflow surface
- **THEN** 渲染结果 SHALL 使用 `/skill:<skillDirName>` 形式（如 `/skill:opsx-archive-change`）
- **AND** `skillDirName` SHALL 来自共享 workflow surface manifest 的显式值
