...
```bash
opsx bootstrap init [--mode <full|opsx-first|refresh>] [--scope <path>] [--restart] [--granularity <coarse|fine>]
```
## Requirements
...
- **GIVEN** `process.stdout.isTTY` 为 `true`
- **AND** 用户传了 `--mode full`
- **WHEN** 执行 `opsx bootstrap init --mode full`
- **THEN** 直接使用指定模式，不提问
...
- **WHEN** 执行 `opsx bootstrap init --mode opsx-first`
- **THEN** 正常执行，不报错

### Requirement: Retained workspace restart SHALL require explicit user intent

`opsx bootstrap init` SHALL 将正常初始化与 completed retained workspace restart 视为两类不同意图。除非用户显式传入 `--restart`，否则系统 SHALL NOT 移动、覆盖或重置已保留的 workspace。

#### Scenario: init rejects completed retained workspace without restart

- **GIVEN** `.opsx/bootstrap/` 已存在
- **AND** workspace 生命周期状态表明上一轮 promote 已完成
- **WHEN** 用户执行 `opsx bootstrap init --mode refresh`
- **THEN** 命令 SHALL fail fast
- **AND** 错误信息 SHALL 指向 `opsx bootstrap init --mode refresh --restart`
- **AND** SHALL NOT 把删除 `.opsx/bootstrap/` 作为默认修复路径
...
- **THEN** 命令 SHALL fail fast
- **AND** 错误信息 SHALL 指向 `opsx bootstrap status` 或 `opsx bootstrap instructions`
...
- **AND** the agent runs `opsx bootstrap init --mode full --granularity coarse`
- **THEN** init SHALL create `.opsx/bootstrap/scope.yaml` with `granularity: coarse`
- **AND** init SHALL NOT rewrite the value to `fine`

#### Scenario: Missing granularity fails fast
- **WHEN** `opsx bootstrap init --mode full` is executed without a granularity value
...
- **WHEN** `opsx bootstrap init --mode full --granularity medium` is executed
- **THEN** init SHALL fail fast with an error listing valid values `coarse` and `fine`
- **AND** init SHALL NOT create or update `.opsx/bootstrap/scope.yaml`

#### Scenario: Restart inherits retained granularity
- **GIVEN** a completed retained workspace has `granularity: coarse` in `scope.yaml`
- **WHEN** the agent runs `opsx bootstrap init --mode refresh --restart` without `--granularity`
...
- **WHEN** the agent runs `opsx bootstrap init --mode refresh --restart --granularity fine`
...
- **WHEN** the user runs `opsx bootstrap advance scan`
