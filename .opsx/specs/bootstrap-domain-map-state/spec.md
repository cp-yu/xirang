...
- **GIVEN** `.opsx/bootstrap/domain-map/dom.auth.yaml` 存在
...
- **AND** `.opsx/bootstrap/domain-map/dom.auth.yaml` 不存在
- **WHEN** 读取 bootstrap state
...
- **AND** `.opsx/bootstrap/domain-map/dom.cli.yaml` contains `spec_groups` whose capabilities all exist in the same domain-map capability list
...
- **GIVEN** `scope.yaml` contains `granularity: coarse`
- **AND** `.opsx/bootstrap/domain-map/dom.cli.yaml` has capabilities but no `spec_groups`
