...
## Purpose
Define `opsx feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `opsx feedback` command that creates a GitHub Issue in the opsx repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `opsx feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the opsx repository
...
- **WHEN** user executes `opsx feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (OPSX version, platform, timestamp)
...
- **WHEN** user runs `opsx feedback "message"`
...
- **WHEN** user has disabled telemetry via `OPSX_TELEMETRY=0`
- **AND** user runs `opsx feedback "message"`
...
- **AND** submits via `opsx feedback` command on user confirmation
...
- **WHEN** user types `opsx fee<TAB>`
- **THEN** the shell completes to `opsx feedback`
