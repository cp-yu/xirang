...
`opsx candidate init` SHALL 根据一个明确的起点创建唯一 active `.opsx/candidate/` workspace：current formal OPSX、clean skeleton 或用户指定路径。
...
`opsx candidate status` SHALL 在不修改 Candidate、formal source 或 history 的情况下，计算 active workspace state、starting point、source inventory、history count 和 disk usage。
...
- **AND** SHALL 指引调用方运行 `opsx candidate init`

### Requirement: Candidate validation SHALL 确定性且只读
`opsx candidate validate` SHALL 在不写入任何 workspace file 的情况下，对 `build.md`、Candidate Architecture 和 Candidate Specs 执行 canonical representation 与 Semantic Closure validation。valid result SHALL 包含绑定精确 reviewed bytes 的 SHA-256 `reviewDigest`。
...
- **THEN** `.opsx/candidate/`、`.opsx/architecture/`、`.opsx/specs/` 和 `.opsx/history/` 下的 bytes SHALL 保持不变
- **AND** SHALL NOT 持久化 validation result 或 digest file

### Requirement: Candidate promotion SHALL 要求当前 confirmed digest
`opsx candidate promote --digest <reviewDigest>` SHALL 创建 immutable Candidate snapshot、重新执行 validation，并且只有 supplied digest 与新计算 digest 完全一致时才允许 promotion。
...
Promotion 在替换 formal source 前 SHALL 创建包含 `build.md`、`promotion.yaml` 和完整 previous Architecture + Specs 的 immutable `.opsx/history/builds/<build-id>/` entry，然后以一个可恢复 transaction 替换完整 formal trees。
...
- **AND** `.opsx/architecture/` 与 `.opsx/specs/` SHALL 与 Candidate target trees 完全一致，包括删除 Candidate 中不存在的 stale formal files
