...
`opsx validate --type change` 和 `opsx validate --changes` 在验证 change 时 SHALL 并行执行 `validateChangeDeltaSpecs()` 和 `validateOpsxDelta()`。

#### Scenario: 单 change 验证合并两个报告

- **WHEN** 执行 `opsx validate <change-name> --type change`
