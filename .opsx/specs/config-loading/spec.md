...
## Purpose
Define how `.opsx/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.
## Requirements
...
- **WHEN** `.opsx/config.yml` exists and `.opsx/config.yaml` does not exist
- **THEN** system reads from `.opsx/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `.opsx/config.yaml` and `.opsx/config.yml` exist
- **THEN** system prefers `.opsx/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `opsx new change`
...
`.opsx/config.yaml` 的已验证字段 SHALL 以稳定、可组合的形式暴露给配置投影层，使 prompt projection 与 runtime projection 共享同一份 source-of-truth 输入，而不是各自重新读取和解释原始 YAML。
...
`.opsx/config.yaml` 的项目配置加载器 SHALL 解析顶层 `git` 节点，并支持 `commitMessage.boundary`、`commitMessage.archive`、`commitMessage.merge`、`merge.strategy`、`branch.deleteAfterArchive` 字段。`commitMessage.*` 三个字段 SHALL 为可选的项目根相对 POSIX 路径，指向用户自有 commit message 模板，无默认值。
...
- **WHEN** config 包含：
  ...
  git:
    commitMessage:
      archive: .opsx/references/my-archive.md
      merge: .opsx/references/my-merge.md
    merge:
      strategy: no-ff
    branch:
...
- **THEN** 加载器 SHALL 输出 warning 指明 convention 字段已废弃、模板由 `.opsx/references/` 与 `git.commitMessage.*` 路径覆盖决定
...
- **AND** SHALL 表达 `commitMessage.*` 已配置时读用户模板、未配置时读 `.opsx/references/` 内置模板的路由语义
...
- **WHEN** 在 Windows 上加载 `opsx\config.yaml`
...
The project config layer SHALL expose a shared default materialization contract for disk writes so `opsx init` and `opsx update` use the same functional defaults for project configuration.
...
- **WHEN** `.opsx/config.yaml` 包含 `propose.smartRouting` 或 `propose.requireExplore`
- **THEN** loader SHALL 继续解析其他有效字段
- **AND** 返回的 ProjectConfig SHALL NOT 包含 `propose`
- **AND** SHALL NOT 输出退役或非法字段 warning
- **AND** SHALL NOT 修改 `.opsx/config.yaml`
