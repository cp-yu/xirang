...
`opsx arch export` SHALL 将 LikeC4 架构图导出为图片文件。

#### Scenario: 导出 PNG 格式

- **WHEN** 运行 `opsx arch export --format png --output docs/architecture/`
...
- **WHEN** 运行 `opsx arch export --format svg --output docs/`
...
- **WHEN** 运行 `opsx arch export --output docs/architecture/`
- **THEN** SHALL 创建目录
- **AND** SHALL 导出图片到该目录
