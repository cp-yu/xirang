...
## Purpose
Define top-level `opsx show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `opsx show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `opsx change show`/`opsx spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `opsx show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `opsx show --type spec <item>`
...
- **WHEN** executing `opsx show <item> --json`
...
- **WHEN** showing a change with `opsx show <change-name> --deltas-only`
