...
- 若写入了 OPSX delta：`opsx/project.opsx.yaml`、`opsx/project.opsx.relations.yaml`、`opsx/project.opsx.code-map.yaml`
...
- **AND** sync 改写 `opsx/project.opsx.yaml`（该文件在 evidenceFiles 中）
...
- **WHEN** 外部进程再次修改 `opsx/project.opsx.yaml`
- **THEN** 后续 `checkFreshness` SHALL 检测到 STALE
- **AND** details SHALL 列出 `opsx/project.opsx.yaml` 为变更文件
