...
## Purpose
Define `/opsx:onboard` behavior for guiding users through an end-to-end OPSX workflow on their real codebase.
## Requirements
### Requirement: OPSX Onboard Skill

The system SHALL provide an `/opsx:onboard` skill that guides users through their first complete OPSX workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/opsx:onboard`
- **THEN** agent checks if OPSX is initialized
- **AND** if not initialized, prompts user to run `opsx init` first
...
- **WHEN** onboarding drafts an OPSX artifact under project config that defines authoring constraints
