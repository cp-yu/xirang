...
#### Scenario: OPSX_TELEMETRY opt-out
- **WHEN** `OPSX_TELEMETRY=0` is set in the environment
...
- **AND** the user sets `OPSX_TELEMETRY=0`
...
- **WHEN** `CI=true` is set
- **AND** `OPSX_TELEMETRY=1` is explicitly set
...
- **WHEN** a user runs their first opsx command
- **AND** telemetry is enabled
- **THEN** the system displays: "Note: OPSX collects anonymous usage stats. Opt out: OPSX_TELEMETRY=0"
