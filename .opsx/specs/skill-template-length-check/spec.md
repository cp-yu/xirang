---
element: cap.validation.skill-template-length
---
...
- **WHEN** `opsx-explore/SKILL.md` 的 default、claude、codex 三个变体均为 541 行
- **THEN** 错误信息 MUST 显示为一行：`• opsx-explore/SKILL.md (default, claude, codex): 541 lines (+341, limit 200)`

#### Scenario: 同一文件的不同变体行数不同时分别显示

- **WHEN** `opsx-verify/SKILL.md` 的 claude 和 codex 变体为 604 行，default 变体为 580 行
- **THEN** 错误信息 MUST 显示为两行：
  - `• opsx-verify/SKILL.md (claude, codex): 604 lines (+404, limit 200)`
  - `• opsx-verify/SKILL.md (default): 580 lines (+380, limit 200)`

#### Scenario: reference 文件单独计算

- **WHEN** `opsx-optimizer/SKILL.md` 为 180 行且 `opsx-optimizer/references/output-protocol.md` 为 501 行
- **THEN** 错误信息 MUST 只报告 `opsx-optimizer/references/output-protocol.md`
- **AND** 测试 MUST NOT 将两个文件相加后报告 `opsx-optimizer` 超标
...
- **WHEN** 某个 skill（如 opsx-verify）针对不同 tool 有不同的生成文件内容
...
- **THEN** 各 workflow skill 声明的长协议 MUST 物化到 `.opsx/references/opsx-<name>.md`
- **AND** `opsx-apply-change` 的 Phase 2 优化协议 MUST 物化为 `.opsx/references/opsx-apply-phase2-optimization.md`
- **AND** `opsx-archive-change` 的 commit message 格式说明 MUST 物化为 `.opsx/references/opsx-boundary-commit-message.md`、`.opsx/references/opsx-archive-commit-message.md` 与 `.opsx/references/opsx-merge-summary-message.md`
