...
系统 SHALL 注册 `opsx bootstrap backfill-specs` 子命令。

#### Scenario: 正常执行

- **WHEN** 执行 `opsx bootstrap backfill-specs`
- **THEN** SHALL 调用 Backfill Engine
- **AND** SHALL 输出已写入和未匹配的统计

#### Scenario: JSON 输出

- **WHEN** 执行 `opsx bootstrap backfill-specs --json`
...
- **WHEN** executing `opsx bootstrap backfill-specs --mappings <file> --json`
