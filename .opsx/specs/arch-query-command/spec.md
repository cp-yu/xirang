...
`opsx arch query <element-id>` SHALL 读取 LikeC4 架构模型并输出指定 element 的详细信息。
...
- **WHEN** 运行 `opsx arch query cap.ai.skill-generation --relations --json`
...
- **WHEN** 运行 `opsx arch query cap.nonexistent.feature`
- **THEN** SHALL 退出码为非零
