...
系统 SHALL 提供 `opsx config project` CLI 命令，读取并返回归一化后的项目配置。输出 MUST NOT 包含已退役的 Propose routing 配置。

#### Scenario: JSON 输出包含完整配置

- **WHEN** 用户执行 `opsx config project --json`
...
- **WHEN** 项目目录中不存在 `.opsx/config.yaml`
- **THEN** 系统输出 `{ "rules": {} }` 作为最小有效配置
- **AND** 以 exit code 0 退出

#### Scenario: 人类可读文本输出

- **WHEN** 用户执行 `opsx config project`（不带 `--json`）
- **THEN** 系统以 YAML-like 格式打印各配置项
- **AND** 输出 SHALL NOT 包含 `propose`
- **AND** 以 exit code 0 退出

#### Scenario: 旧 Propose routing 配置不进入查询结果
- **WHEN** `.opsx/config.yaml` 包含旧 `propose.smartRouting` 或 `propose.requireExplore`
- **THEN** JSON 与人类可读输出 SHALL NOT 包含 `propose`
- **AND** 其他有效配置字段 SHALL 正常输出

### Requirement: 命令集成于 config 子命令体系

`config project` 命令 SHALL 作为 `opsx config` 的子命令注册。

#### Scenario: 帮助信息

- **WHEN** 用户执行 `opsx config --help`
- **THEN** 显示 `project` 作为可用子命令
- **AND** 描述该命令为查询项目级配置

### Requirement: 输出格式与 instructions 配置投影一致

`opsx config project --json` 的输出 SHALL 与 `opsx instructions <artifact> --json` 中的 `configProjection.normalized` 使用相同的 `NormalizedProjectConfig` 数据结构。

#### Scenario: 字段一致性

- **WHEN** 同一个项目中同时执行 `opsx config project --json` 和 `opsx instructions proposal --change "<name>" --json`
- **THEN** `opsx config project --json` 的顶层字段集合与 `instructions` 命令中 `configProjection.normalized` 的字段集合相同
