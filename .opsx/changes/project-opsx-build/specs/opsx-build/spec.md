...
`opsx-build` SHALL 从已配置的 OPSX 项目开始，获取用户的探索范围和明确约束，并在编写 Candidate source 前将这些输入记录到 `.opsx/candidate/build.md`。

#### Scenario: 用户选择探索范围
- **WHEN** `opsx-build` 在没有 active Candidate 时启动
...
当当前 formal OPSX 存在时，`opsx-build` SHALL 展示当前 OPSX 概览，并在调用 `opsx candidate init` 前获得明确的 Candidate 起点选择。
...
`opsx-build` SHALL 同时编写完整的 Candidate LikeC4 modules 和 singular element-bound Candidate Specs。`build.md` SHALL 只是临时 compilation scaffolding，不得定义 durable graph 或 contract facts。

#### Scenario: Candidate 同时包含 Architecture 和 Specs
- **WHEN** Agent 已解决所需的 semantic decisions
- **THEN** SHALL 编写 `.opsx/candidate/architecture/` 和 `.opsx/candidate/specs/`，使其共同组成一个 Candidate
...
`opsx-build` SHALL 允许 Agent 自由选择探索顺序和可选 subagents。Agent SHALL 使用 `opsx candidate validate` 的 diagnostics 修复 Candidate，并在 promotion 前直接向用户呈现 valid Candidate 和 review digest。
...
- **WHEN** `opsx candidate validate` 返回 valid 的 `reviewDigest`
- **THEN** Agent SHALL 向用户呈现 Candidate summary、formal diff、重要 decisions 和 digest
- **AND** 只有在用户确认该版本后，Agent 才 SHALL 调用 `opsx candidate promote --digest <reviewDigest>`
