...
#### Scenario: [ADDED] Optional contract 缺失
- **WHEN** element kind 的 `contractPolicy` 为 `optional`
- **AND** element 没有 Spec
- **THEN** SHALL NOT 产生 issue

#### Scenario: [REMOVED] 检查 capabilityId 字段

- **GIVEN** 一个 capability 的 metadata 缺少 `capabilityId`
- **WHEN** 运行语义验证
- **THEN** SHALL 返回警告 "Capability <element-id> missing capabilityId in metadata"

#### Scenario: [REMOVED] 检查 specs 路径存在性

- **GIVEN** capability metadata 包含 `specs ['.opsx/specs/nonexistent.md']`
- **AND** 该文件不存在
- **WHEN** 运行语义验证
- **THEN** SHALL 返回警告 "Spec file not found: .opsx/specs/nonexistent.md"
...
#### Scenario: [MODIFIED] 验证通过时无 errors
