...
- **GIVEN** 项目包含 `openspec/project.opsx.yaml` 和 `openspec/project.opsx.relations.yaml`
- **WHEN** 运行 `openspec migrate opsx-to-likec4`
...
迁移工具 SHALL 生成 `openspec/architecture/` 目录，包含 specification.c4、按 domain 组织的模型文件和 views.c4。

#### Scenario: 生成标准目录结构

- **GIVEN** OPSX 模型包含 9 个 domains 和 115 个 capabilities
- **WHEN** 运行 `openspec migrate opsx-to-likec4`
- **THEN** SHALL 创建 `openspec/architecture/specification.c4`
- **AND** SHALL 创建 `openspec/architecture/domains/` 目录
- **AND** SHALL 为每个 domain 创建 `domains/<domain-id>.c4` 文件
- **AND** SHALL 创建 `openspec/architecture/views.c4`
...
- **AND** 存在 `openspec/specs/skill-generation/spec.md`
- **WHEN** 转换该 capability
- **THEN** SHALL 在 metadata 中添加 `specs ['openspec/specs/skill-generation/spec.md']`

#### Scenario: 推断新格式多个 .md 文件

- **GIVEN** capability ID 为 `cap.apply.task-executor`
- **AND** 存在 `openspec/specs/task-executor/phase0.md` 和 `phase1.md`
- **WHEN** 转换该 capability
...
- **WHEN** 生成 `views.c4`
- **THEN** SHALL 包含 `view index { title 'OpenSpec Architecture'; include * }`
...
- **WHEN** 运行 `openspec migrate opsx-to-likec4 --dry-run`
...
- **WHEN** 迁移完成
- **THEN** SHALL 运行 `npx likec4 validate openspec/architecture/`
...
- **GIVEN** 存在 `openspec/project.opsx.yaml` 和 `openspec/project.opsx.relations.yaml`
- **WHEN** 运行 `openspec migrate opsx-to-likec4`
