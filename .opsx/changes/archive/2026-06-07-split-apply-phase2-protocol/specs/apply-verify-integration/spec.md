...
Phase 1 或 Phase 3 失败 SHALL 回到 Phase 0 recovery loop。系统 SHALL 仅在同一 task 的同一 normalized error signature 经 master remediation 后连续失败 2 次时 pause。
...
`openspec-apply-change` skill 的主 `SKILL.md` SHALL 将 Phase 2 详细协议放在 `references/apply-phase2-optimization.md`，并在主指令中要求 agent 在执行 Phase 2 前读取该 reference。该 reference SHALL 保留 apply 的栈式 checkpoint 语义，明确 checkpoint 是 git stash entry，SHALL NOT 指示 agent 使用 `git tag apply-opt-checkpoint-*` 创建 checkpoint。
...
#### Scenario: Phase 1 失败进入 recovery loop
...
- **AND** SHALL 将 remediation 映射回受影响 task
- **AND** SHALL 回到 Phase 0 recovery loop 要求主 agent 修复
- **AND** SHALL NOT 因首次 Phase 1 失败直接询问用户
...
#### Scenario: apply skill 引用 Phase 2 reference

- **WHEN** 调用 `getApplyChangeSkillTemplate()`
- **THEN** 返回的 SkillTemplate SHALL 包含 `referenceFiles` 数组
- **AND** 数组 SHALL 包含一个 reference，其 `path` 为 `references/apply-phase2-optimization.md`
- **AND** 主 `instructions` SHALL 明确要求执行 Phase 2 前读取 `references/apply-phase2-optimization.md`

#### Scenario: Phase 2 reference 明确 stash checkpoint 命令

- **WHEN** 读取 `references/apply-phase2-optimization.md`
- **THEN** content SHALL 包含 `git stash push -u -m "apply-opt-checkpoint-r0"`
- **AND** content SHALL 包含 `git stash push -u -m "apply-opt-checkpoint-r<N>"`
- **AND** content SHALL 包含 `git stash apply stash@{0}`
...
#### Scenario: Phase 3 Seal 失败进入 recovery loop

- **WHEN** `openspec verify seal <change-name>` 失败
- **THEN** 系统 SHALL 保留 seal 诊断
- **AND** SHALL 将 seal failure 转换为 remediation context
- **AND** SHALL 将 remediation 映射回受影响 task
- **AND** SHALL 回到 Phase 0 recovery loop
- **AND** SHALL NOT 因首次 seal failure 直接 pause
