...
**Reason**: Profile 系统已删除，固定安装 5 个工作流
**Migration**: 无需迁移。`openspec init` 现在固定安装 propose, explore, apply, archive, bootstrap-opsx 五个工作流的 skills。

### Requirement: Skill generation per tool
**Reason**: Profile 系统已删除，不再基于 profile 选择工作流
**Migration**: 所有工具固定生成 5 个工作流的 skills，无需配置。

### Requirement: Command generation per tool
**Reason**: 该 requirement 的 profile 相关场景已删除
**Migration**: 命令生成现在基于固定的 5 个工作流，不再依赖 profile 配置。

### Requirement: Smart defaults init flow
**Reason**: Profile 默认值场景已删除
**Migration**: `openspec init` 不再使用或显示 profile 设置，固定安装 5 个工作流。
