...
系统 SHALL 提供 `opsx diff --change <name>`，从同一 immutable Formal snapshot 将 active change materialize 为 Target Semantic Model，并输出 Formal 与 Target 的完整 semantic text diff。默认执行 SHALL 只读且 SHALL NOT 创建或修改文件。

#### Scenario: 默认输出完整文本 diff
- **WHEN** 用户运行 `opsx diff --change payment-update`
...
- **WHEN** 用户指定不存在或已进入 `.opsx/changes/archive/` 的 change
...
`opsx diff` SHALL 支持 text、scope-filtered 与 JSON projections，所有 projections MUST 来自同一份 Diff IR，并保持 identity、operation、summary 与 diagnostics 一致。

#### Scenario: 只显示 Specs
- **WHEN** 用户运行 `opsx diff --change payment-update --scope specs`
...
- **WHEN** 用户运行 `opsx diff --change payment-update --scope architecture`
- **THEN** 命令 SHALL 只渲染 Architecture entries 与 Architecture diagnostics

#### Scenario: JSON 输出完整 Diff IR
- **WHEN** 用户运行 `opsx diff --change payment-update --json`
...
`opsx diff --change <name> --write` SHALL 在输出当前 text diff 的同时，以原子方式生成 `.opsx/changes/<name>/effective-change.md`。该文件 SHALL 是 deterministic generated review artifact，MUST NOT 作为 validation、sync 或 target materialization 输入。

#### Scenario: 写入有效报告
- **WHEN** change validation 通过且用户运行 `opsx diff --change payment-update --write`
...
- **WHEN** 在 Windows、macOS 或 Linux 重复运行 `opsx diff --change payment-update --write`
...
- **AND** 用户运行 `opsx diff --change payment-update --write`
