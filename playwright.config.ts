...
export default defineConfig({
  ...
  projects: [
    {
      name: 'desktop',
      grep: /browses Element Contracts|opens an Authored View|keeps View source|browses Model View|handles ADDED projection|refreshes the model through HMR|renders each Change presentation mode|expands in place|browses the complete candidate model|reviews candidate changes in diff only mode|keeps invalid candidate sources diagnosable|shows candidate cards on the landing page|applies kind presentation|keeps operation color|exports a hierarchy tree|exports the current focus|exports a change source|exports without a snapshot|exports the complete model structure for file formats/,
