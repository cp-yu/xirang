...
export default defineConfig({
  ...
  projects: [
    ...
    {
      name: 'mobile',
      grep: /mobile Contract dialog|browses Model View|handles ADDED projection|opens ADDED element details|refreshes the model through HMR|renders each Change presentation mode|fits the diagram after switching|renders four-state diff visuals|browses the complete candidate model|reviews candidate changes in diff only mode|keeps invalid candidate sources diagnosable|shows candidate cards on the landing page|opens an active change from the landing page|applies kind presentation|keeps operation color|exports a hierarchy tree|exports the current focus|exports a change source|exports without a snapshot|exports the complete model structure for file formats/,
