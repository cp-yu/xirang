...
describe('artifact-graph workflow integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'xirang-workflow-test-'));
  });
  ...
  describe('semantic-model workflow', () => {
    it('should progress through complete workflow', () => {
      // 1. Resolve the real built-in schema
      const schema = resolveSchema('semantic-model');
      const graph = ArtifactGraph.fromSchema(schema);

      // Verify schema structure
      expect(graph.getName()).toBe('semantic-model');
