...
describe('xirang verify command', () => {
  let tempDir: string;
  ...
  it('rejects incomplete reconciliation and illegal pending-to-resolved transitions atomically', async () => {
    ...
    const before = await fs.readFile(path.join(tempDir, '.xirang', 'changes', 'c1', '.verify-result.json'), 'utf-8');
    ...
    expect(await fs.readFile(path.join(tempDir, '.xirang', 'changes', 'c1', '.verify-result.json'), 'utf-8')).toBe(before);
  });
  ...
  it('reports git HEAD transitions as information without making freshness stale', async () => {
    ...
    const phase1Result = JSON.parse(
      await fs.readFile(path.join(tempDir, '.xirang', 'changes', 'c1', '.verify-result.json'), 'utf-8')
    );
  ...
  it('formats verify gate failures with Required Corrections guidance', async () => {
    ...
    expect(status.stdout).toContain('xirang verify phase1 c1');
    expect(status.stdout).toContain('xirang sync c1 --no-verify');
  });
