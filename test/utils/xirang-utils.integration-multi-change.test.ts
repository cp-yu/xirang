...
function applyDelta(bundle: ProjectXirangBundle, delta: XirangDelta): ProjectXirangBundle {
...
async function writeChangeArtifacts(testDir: string, changeName: string, delta: XirangDelta): Promise<void> {
  const changeDir = path.join(testDir, '.xirang', 'changes', changeName);
...
describe('Integration: Multiple Changes on Real Project', () => {
  ...
  const seedBundle: ProjectXirangBundle = {
