...
#### Scenario: 新 change
- **WHEN** 目标 change 不存在
- **THEN** snack SHALL 执行 `openspec new change "<name>"`
...
- **AND** SHALL NOT 将其当作 OpenSpec CLI flag
...
Snack SHALL 运行 full change validation。出现 ERROR/WARNING 时 SHALL 修复一轮并复检一次。最终 validation 后 SHALL 执行 `openspec scenario-labels "<name>" --write`，且 SHALL NOT 仅因 labels 再运行 validate。
