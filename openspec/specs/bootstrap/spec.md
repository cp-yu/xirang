...
Bootstrap SHALL 为完整生命周期文件提供结构化定义，并由 `openspec bootstrap instructions <phase> --json` 按 phase 投影。Definitions SHALL 区分 workflow state、retained bootstrap authoring inputs、CLI-generated derived review projections、review-controlled gates 与 workflow-managed durable source。
...
- **AND** SHALL 将 `openspec/specs/**/*.md` 定义为 durable behavior source
