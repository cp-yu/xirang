...
## Purpose
定义注入 workflow skills 与审查 subagents 的最小 OpenSpec 项目哲学，使所有消费者共享一致的 semantic source、scaffolding、source completeness 与 Agent compiler 定义，而不混入具体工作流操作规则。

## Requirements

### Requirement: OpenSpec Philosophy 片段定义

系统 SHALL 在 `src/core/templates/fragments/opsx-fragments.ts` 中提供 `OPENSPEC_PHILOSOPHY` 共享片段常量。该片段 SHALL 以英文书写，不受 `proseLanguage` projection 管辖，并 SHALL 只表达以下项目定义：

1. OpenSpec 是 human intent 与通用编程语言之间的 human-intent programming layer。
...
### Requirement: OpenSpec Philosophy 注入 workflow skill 模板

6 个用户可调用 workflow skill 模板 SHALL 在首句职责描述之后、操作步骤之前注入 `OPENSPEC_PHILOSOPHY`：
...
注入 SHALL 使用 TypeScript 模板字符串插值 `${OPENSPEC_PHILOSOPHY}`，并与后续内容保留一个空行。

#### Scenario: 6 个 workflow skills 共享同一片段
- **WHEN** 生成任一受管 workflow skill
- **THEN** instructions SHALL 包含完整的 `OPENSPEC_PHILOSOPHY`
- **AND** 各 workflow MUST NOT 内联另一份改写后的项目哲学

### Requirement: OpenSpec Philosophy 注入 internal subagent 模板

`reviewer` 与 `optimizer` internal subagent SHALL 在 Role 段之后注入 `OPENSPEC_PHILOSOPHY`。`impact-sweeper`、`feedback` 与 `verify-execution-model` SHALL NOT 注入该片段。

#### Scenario: reviewer 与 optimizer 包含共享哲学
- **WHEN** reviewer 或 optimizer prompt 被生成
- **THEN** prompt SHALL 在 Role 与 Hard Constraints 之间包含完整的 `OPENSPEC_PHILOSOPHY`

#### Scenario: 排除 surface 不包含共享哲学
- **WHEN** impact-sweeper prompt、feedback template 或 verify-execution-model helper 被生成
- **THEN** 输出 SHALL NOT 包含 `OpenSpec Philosophy` 标题

#### Scenario: 排除决策编码在片段注释中
- **WHEN** `OPENSPEC_PHILOSOPHY` 常量在 `opsx-fragments.ts` 中被定义
