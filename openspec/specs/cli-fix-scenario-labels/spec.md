# cli-fix-scenario-labels Specification

## Purpose
This specification records behavior introduced by change add-fix-scenario-labels-command. Replace this Purpose with the formal capability intent before archive.
## Requirements
### Requirement: Scenario label fix command

`openspec fix-scenario-labels` SHALL provide a deterministic preview/write command that derives change-local scenario operation labels from the difference between main specs and change-local `## MODIFIED Requirements` blocks.

#### Scenario: Preview reports suggested scenario labels

- **WHEN** executing `openspec fix-scenario-labels my-change --preview`
...
- **WHEN** executing `openspec fix-scenario-labels my-change --preview --json`
...
- **WHEN** executing `openspec fix-scenario-labels my-change --write`
...
- **WHEN** executing `openspec fix-scenario-labels missing-change --preview`
- **THEN** the command SHALL fail with an unknown change error
- **AND** SHALL NOT create files

#### Scenario: Command is idempotent

- **GIVEN** `openspec fix-scenario-labels my-change --write` has already applied all suggestions
...
The scenario label fixer SHALL derive labels by comparing label-free scenario titles and scenario bodies between the main spec requirement and the change-local MODIFIED requirement.
...
- **WHEN** scenario labels are fixed
