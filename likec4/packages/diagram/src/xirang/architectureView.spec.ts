...
import { isXirangSpecDiagnostic, type XirangRuntimeVariant } from './SpecLoaderContext'
...
describe('materializeXirangArchitectureView', () => {
  it('partitions Spec diagnostic paths from Architecture diagnostics', () => {
    expect(isXirangSpecDiagnostic('specs/auth/spec.md')).toBe(true)
    expect(isXirangSpecDiagnostic('.xirang/changes/demo/specs/auth/spec.md')).toBe(true)
    expect(isXirangSpecDiagnostic('architecture-delta.c4')).toBe(false)
    const withBindingError: XirangRuntimeVariant = {
      ...variant,
