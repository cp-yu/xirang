...
import type { OpsxDiffEntry, OpsxDiffOperation, OpsxRuntimeVariant } from './SpecLoaderContext'

type ViewNode = DiagramView['nodes'][number]
type ViewEdge = DiagramView['edges'][number]
type Architecture = NonNullable<OpsxRuntimeVariant['architecture']>
type ArchitectureElement = Architecture['elements'][number]
type ArchitectureRelation = Architecture['relations'][number]

const operationColor: Record<OpsxDiffOperation, ViewNode['color']> = {
  ADDED: 'green',
  MODIFIED: 'amber',
  REMOVED: 'red',
}

function architectureEntries(variant: OpsxRuntimeVariant): OpsxDiffEntry[] {
...
export function materializeOpsxArchitectureView(
  formal: DiagramView,
  variant: OpsxRuntimeVariant,
  mode: 'full' | 'diff',
): DiagramView {
  ...
  return {
    ...formal,
    hash: `${formal.hash}:opsx:${variant.architectureFingerprint ?? variant.id}:${mode}`,
    nodes,
    edges,
