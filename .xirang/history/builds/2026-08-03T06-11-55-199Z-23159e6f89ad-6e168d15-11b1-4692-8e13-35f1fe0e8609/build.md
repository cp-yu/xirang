...
## legacy specs 实现细节补入（2026-08-03 第三轮裁决）

用户裁决：`.xirang/specs/` 中仍在实现中的细节也必须进入 Candidate，且以 subagent 编辑完成（不使用程序化批量插入）。4 个 writer 按元素域分组并行补齐：

- **Writer A（CLI/确定性操作域）**：validation-commands.md（Validation 保持只读 +1 Scen）、cli.md（--port +1 Scen）、schema-resolution.md（跨平台内置路径解析 +1 Scen）。Writer A 最初新增的「Task Verifies scenario label 归一化」因与 Candidate 既有 label 拒绝语义（`validation-commands` Scenario operation label 校验）矛盾，在复审中被删除，未保留。
- **Writer B（配置/投影/工具集成域）**：references-home.md（内联引用 project-root 标记 +1 Scen）。Writer B 在 workflow-templates.md 新增的「CLI 点查询互补定位」已被用户 2026-08-03 手动回退（删除），该文件保持 HEAD 版本，未纳入。
- **Writer C（过程/验证域）**：branch-isolation.md（源工作区清理 tracked/untracked 区分、worktree 内执行全部阶段、未提交路径入证据 scope +3 Req/5 Scen）、optimizer-protocol.md（scope 导航基于 baseCommit diff +1 Req/2 Scen）、reviewer-protocol.md（WARNING 降级条件 +1 Req/2 Scen）、change-archive.md（boundary commit Changes/Implementation footer +1 Req/2 Scen）、optimizer-findings.md（priorityReason 优先理由 +1 Req/1 Scen）。
- **Writer D（模型/身份域）**：semantic-browser.md（按需安全读取 Contract 单元 +1 Req/5 Scen，由 spec-content-browser 迁移：Spec file/registry → Element identity/Contract 单元）。

处置统计：4 个 writer 共 111 specs；净新增 9 处实质缺口（9 Requirement + 22 Scenario，与 git diff 实际计数一致），其余 covered（术语迁移后已由既有 Candidate 覆盖）或 retired（architecture-delta.c4、Spec store/registry/frontmatter、cli-change/diff/spec 命令族、archive 内嵌 sync/--no-sync、impact-sweeper、implementer、profiles、bootstrap schema 等，正向语义未写入）。新增内容均以代码/测试证据为准（如 `src/cli/index.ts` 的 --port、`src/core/archive.ts` 的 boundary commit 行为、identity-keyed manifest lookup 的 Contract 端点）。

## 正式模型 5 天演进纳入（2026-08-03 第四轮裁决）

用户裁决：最近五天 `.xirang/model/` 的修改（7 个 commit）包含比 `.xirang/specs/` 更新的内容，需纳入 Candidate。父代理全量对比 formal vs Candidate 后，差异收敛为 3 项，由 writer 以 agent 能力完成：

1. **三个顶层 Perspective 命名对齐正式模型**：`semantic-object` → `semantic-objects`（复数）、`process-dimension` → `realization-process`、`collaboration-dimension` → `collaboration-structure`；definition 更新为正式模型文本；8 个 child 的 parent（semantic-model/change/framework-identity → semantic-objects；semantic-model-build/change-realization/delivery-validation → realization-process；participants/interaction-surfaces → collaboration-structure）及正文引用全部同步；relationships 无旧 identity 引用。
2. **semantic-browser 补「优先使用 Authored scoped View」Requirement**（含 Scenario「根 Element 具有 Authored scoped View」），逐字复制自正式模型 5 天演进内容；Element-derived View 表述保留正式文本（作为导航优先级语义，不引入持久化元素）。
3. **derived-views「不持久化」补 Scenario「生成可重建运行表示」**，逐字复制自正式模型。

其余 formal 5 天演进内容（model-view 就地展开/批量展开、cli model-driven authoring help 7 scenarios、deterministic-operations Show/List 四分区编译、change-derived-views 声明 delta、合并呈现 Requirement 差异等）经逐 requirement/逐 scenario 对比已在 Candidate 中完整覆盖，未重复添加。正式模型 `metamodel/perspective.md` 仍带 parents/children 白名单（旧体系），Candidate 保持无白名单（round-2 用户裁决），不受对齐影响。
