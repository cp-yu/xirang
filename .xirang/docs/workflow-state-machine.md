# OPSX 完整工作流状态转移：Propose → Apply → Verify → Sync → Archive
...
```
   ...
   • 存在 delta specs 或        • 用户执行 /xirang:archive
     opsx-delta.yaml            • .verify-result.json FRESH
   • 用户执行 /xirang:sync           + archive-compatible
     (或 archive 内嵌调用)
...
```
                          /xirang:propose "<name>" 或 /xirang:propose (交互式)
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
                    │  P1. INPUT_RESOLVE                   │
                    ...
                    │  P2. SCAFFOLD                        │
                    │  ─────────────────                   │
                    │  xirang new change "<name>"        │
                    │  → 创建 .xirang/changes/<name>/     │
                    │  → 生成 .xirang.yaml (schema元数据) │
                    └──────────────────┬───────────────────┘
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
            ...
            │  │  → 写入 .xirang/changes/<name>/     │         │
            │  │  → 标记工件 status = done            │         │
            │  └─────────────────┬───────────────────┘         │
            │                    │                              │
            │                    ▼                              │
            │  ┌─────────────────────────────────────┐         │
            │  │ 4e. xirang status --change --json  │         │
            │  │  → 重新加载工件状态                  │         │
            │  └─────────────────┬───────────────────┘         │
            │                    │                              │
            │      ┌─────────────┴─────────────┐                │
...
| 条件 | 必须? | 说明 |
|------|-------|------|
| `applyRequires` 全部工件 `done` | ✅ 是 | `xirang status --json` 返回 `isComplete` 或所有必备工件完成 |
...
```
                          /xirang:apply "<name>" 或 /xirang:apply (交互式)
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
                    │  A1. CHANGE_SELECT                   │
    ...
    │  若全部完成 → 建议 archive        │ │ 后续: /xirang:continue │
    │                                  │ │   或 /xirang:apply     │
    │  输出: "建议运行 /xirang:verify"    │ │                      │
    └──────────────────┬───────────────┘ └──────────────────────┘
                       │
                       ▼
              ┌────────────────┐
...
```
  暂停原因                    检测方式                      恢复方式
  ────────                    ────────                      ────────
  任务描述不明确              agent 判断无法确定            /xirang:apply (澄清后继续)
                             实现方向
  实现揭示设计缺陷            实现过程中发现 design.md      /xirang:continue (更新 design 后)
                             与实际代码架构冲突
  遇到技术错误/阻塞           编译错误、依赖缺失、           /xirang:apply (解决阻塞后)
                             不可预见的障碍
  用户主动中断                用户发送中断信号              /xirang:apply (随时继续)
```
...
```
                          /xirang:verify "<name>" 或 /xirang:verify (交互式)
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
                    │  V0. CHANGE_SELECT                   │
...
```
                          /xirang:sync "<name>" 或 archive 内嵌调用
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
                    │  S1. ASSESS                          │
                    ...
                    │                                      │
                    │  检查 .xirang/changes/<name>/specs/ │
                    │  → findSpecUpdates()                 │
                    │  → 发现所有 delta spec → 主spec 映射 │
                    │  → 解析 ADDED/MODIFIED/REMOVED/      │
                    │    RENAMED 操作                      │
                    │                                      │
                    │  检查 .xirang/changes/<name>/       │
                    │  opsx-delta.yaml                     │
...
```
  扩展模式:  /xirang:sync → 独立执行 → /xirang:archive
  核心模式:  /xirang:archive → 内嵌 sync → 移动目录
```
...
```
                          /xirang:archive "<name>" 或 /xirang:archive (交互式)
                                       │
                                       ▼
                    ┌──────────────────────────────────────┐
                    │  AR1. CHANGE_SELECT                  │
                    ...
                    │  ─────────────────                   │
                    │                                      │
                    │  mkdir -p .xirang/changes/archive   │
                    │                                      │
                    │  生成目标名:                          │
...
```
  阻塞条件                              检测方式                  用户操作
  ────────                              ────────                  ────────
  .verify-result.json 不存在            AR2 文件检查              运行 /xirang:verify
...
```
               ...
               └──── 暂停条件 ────┐
                    ...
                    用户中断       │
                                  │
                    /xirang:continue 或 /xirang:apply
```
...
```
  ...
  project.xirang.yaml       R         R         R         R+W       R
  project.xirang.relations  R         R         R         R+W       R
  config.yaml             -         R         R         -         R
  源代码                   -         W         R         -         -
