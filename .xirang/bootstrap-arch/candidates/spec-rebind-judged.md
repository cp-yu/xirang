...
| `arch-query-command` | `…/cli/cap.cli.architecture-navigation` | `cap.cli.arch-query` | high | Requirements define the xirang arch query CLI surface (elementId/FQN, --relations, --depth, --json), matching cap.cli.arch-query rather than the LikeC4 reader engine. |
...
| `artifact-workflow-status` | `cap.artifact-graph.workflow-compilation` | `cap.cli.list` | medium | Sole requirement: xirang list --json adds per-change verifyStatus (FRESH/STALE/MISSING) while preserving status. CLI list surface is the fine owner; state-tracking/freshness are dependencies not the contract owner. |
...
| `cli-completion` | `…/cli/cap.cli.command-discovery` | `cap.cli.completion` | high | Requirements cover shell completion generate/install and native shell UX for xirang completion, owned by cap.cli.completion; introspect is only the reflection helper. |
| `cli-config` | `…/cli/cap.cli.project-setup` | `cap.cli.config` | high | Requirements define the full xirang config CLI (path/list/get/set/unset/reset/edit/project), matching cap.cli.config rather than the project-query sub-capability alone. |
...
| `config-project-query` | `cap.cli.project-setup` | `cap.cli.config.project` | high | Spec is xirang config project query of NormalizedProjectConfig; cli.c4 defines cap.cli.config.project exactly for that CLI surface. project-setup is coarse/wrong. |
...
| `init-project-structure` | `cap.cli.project-setup` | `cap.cli.init` | high | xirang setup/init creates LikeC4 architecture skeleton, Project Root, metamodel, specs dir; matches cap.cli.init description. project-setup is coarse; cap.cli.init-opsx-skeleton is deprecated legacy YAML path. |
...
| `opsx-apply-skill` | `cap.apply.verification-integration` | `cap.apply.verify-integration` | high | Defines /xirang:apply consuming .verify-result.json, remediation sections, and seal→archive CTA; matches cap.apply.verify-integration and the apply-verify-integration auto-map rename (verification-integration → verify-integration). |
| `opsx-archive-skill` | `cap.change.lifecycle` | `cap.change.archive` | high | /xirang:archive skill: readiness, artifact/task gates, archive-time verify reuse, inline delta sync, move to archive/. Best fine owner is cap.change.archive (not branch-merge or specs-sync alone). |
| `xirang-framework-identity` | `…/framework/cap.framework.identity` | `cap.framework.identity` | high | Requirements define single xirang CLI identity and .xirang durable workspace contract, matching cap.framework.identity; workflow-templates is unrelated. |
| `opsx-impact-sweeper-architecture` | `cap.architecture.semantic-model` | `cap.ai.impact-sweeper` | high | Contract is how Impact sweeper navigates model via xirang arch query, elementId identity, and Spec binding—not metamodel/validator/delta-merger. Aligns with ai-impact-sweeper → cap.ai.impact-sweeper and cap.ai.impact-sweeper description. |
...
| `schema-validate-command` | `cap.schema.workflow-definition` | `cap.schema.validate` | medium | xirang schema validate of YAML/Zod/file defs/templates/DAG is domain schema validation—cap.schema.validate. Alternate fine CLI surface cap.cli.schema also mentions structure checks; domain validate is the finer semantic owner. |
| `schema-which-command` | `cap.cli.artifact-workflow` | `cap.cli.schema` | high | xirang schema which reports package location of built-in schemas; cap.cli.schema documents package location + structure for built-in schemas. cap.cli.schemas is list-only. |
| `semantic-delta-application` | `cap.change.semantic-delta` | `cap.architecture.delta-merger` | high | Active architecture-delta.c4 + Spec delta validate/merge/atomic formal apply matches cap.architecture.delta-merger. Reject cap.xirang.delta-merge (deprecated legacy opsx-delta.yaml). Not under domain.change_workflow caps. |
...
| `verify-aware-apply-instructions` | `cap.cli.artifact-workflow` | `cap.cli.verify-aware-instructions` | high | xirang instructions apply states needs_verify/needs_seal/all_done via verify freshness match cli.c4 verify_aware_instructions. |
...
| `arch-plan-remove-command` | `cap.cli.architecture-navigation` | `—` | medium | Defines xirang arch plan-remove (readonly removal impact, change-aware Handled/Unresolved, text/JSON). Candidates only have arch-query/validate/preview/export—no plan-remove. Suggest to: cap.cli.arch-plan-remove. |
...
| `cli-candidate` | `cap.cli.candidate` | `—` | high | xirang candidate init/status/validate/promote workspace lifecycle; no cap.cli.candidate (or any candidate*) elementId in domains/*.c4; historical pre-candidate CLI model. |
| `cli-diff` | `cap.cli.change-operations` | `—` | high | Dedicated xirang diff semantic Formal↔Target command and effective-change.md writeback; candidates have cap.cli.change but no cap.cli.diff/change-operations; change cap is create/list/ops, not semantic diff IR. |
| `xirang-build` | `cap.architecture.bootstrap` | `—` | medium | Project Build (candidate authoring, build.md, validate/promote) is formal identity architecture.bootstrap, but candidates only offer deprecated cap.xirang.bootstrap (legacy scan/map/promote) plus architecture.likec4-reader/semantic-validator/delta-merger/metamodel—none describe Agent-driven Project Build. Do not map to cap.xirang.bootstrap. |
...
- `arch-plan-remove-command`: Defines xirang arch plan-remove (readonly removal impact, change-aware Handled/Unresolved, text/JSON). Candidates only have arch-query/validate/preview/export—no plan-remove. Suggest
...
- `cli-candidate`: xirang candidate init/status/validate/promote workspace lifecycle; no cap.cli.candidate (or any candidate*) elementId in domains/*.c4; historical pre-candidate CLI model.
- `cli-diff`: Dedicated xirang diff semantic Formal↔Target command and effective-change.md writeback; candidates have cap.cli.change but no cap.cli.diff/change-operations; change cap is create/lis
- `xirang-build`: Project Build (candidate authoring, build.md, validate/promote) is formal identity architecture.bootstrap, but candidates only offer deprecated cap.xirang.bootstrap (legacy scan/map/
