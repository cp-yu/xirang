...
apply-change skill SHALL 在实现前扫描 tasks.md 的 Goal、Files、Requirements 与 Checks，检测 task 间互斥声明，以及 task 与 canonical change-local Contracts、design.md 或 Semantic Delta 的冲突。Scenario anchors SHALL 直接使用无 operation metadata 的 exact canonical title。
...
#### Scenario: 检测到 task 需求与 Contract 冲突
...
- **AND** change-local Contract 包含 `#### Scenario: 已调整场景`
...
- **AND** 用户确认后系统 SHALL 继续进入所选方法 reference
