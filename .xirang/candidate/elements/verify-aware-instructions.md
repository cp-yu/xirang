...
`xirang instructions apply` 命令在判定 change 的 `state` 时 SHALL 集成 verify freshness 检查，而非仅依赖 task checkbox 完成度。
...
- **WHEN** `tasks.md` 中所有 checkbox 均为 `[x]`
- **AND** `.verify-result.json` 存在且 freshness 为 FRESH
- **AND** archive compatibility 为 `{ compatible: true }`
- **THEN** `state` SHALL 为 `'all_done'`

#### Scenario: Phase1 PASS 但 Phase2 未完成

- **WHEN** `tasks.md` 中所有 checkbox 均为 `[x]`
- **AND** `.verify-result.json` 存在且 `result` 为 PASS 或 PASS_WITH_WARNINGS
