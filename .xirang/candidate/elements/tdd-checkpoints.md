...
Apply Master Agent 在读取任务的 Goal、Files、Requirements、Checks 后，开始编写测试前，MUST 评估目标接口的可测试性设计：依赖通过参数注入而非在内部创建；函数返回值而非产生副作用；接口面积最小化。若接口设计违反上述原则，Agent MUST 调整接口设计后再继续，而非强行为不可测试的接口编写测试。
...
Apply Master Agent 在为 behavior/code Check 添加或更新测试后，运行 Check 前，MUST 验证测试符合质量标准：测试通过公共接口验证行为，不测试私有方法或内部实现细节；测试不 mock 内部协作者，仅 mock 系统边界；每个测试包含一个逻辑断言；测试在内部重构后仍能通过。若测试违反上述标准，Agent MUST 重写测试后再进入 RED 阶段。
