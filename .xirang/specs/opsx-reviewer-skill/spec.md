...
`xirang-reviewer` skill SHALL 将 subagent 定义为 Phase 1 验证审查者，拥有所有 completeness、correctness、coherence 判定权，且 MUST 遵循以下硬约束：
...
`xirang-reviewer` skill SHALL 定义顶层 agent MUST 传入的轻量定位信息：
...
- **opsxContext**: 自行读取 `changeDir/architecture-delta.c4`、`projectRoot/.xirang/architecture/**/*.c4` 与 element-owned Specs
...
`xirang-reviewer` skill SHALL 为每个 delta spec requirement 定义并强制执行 6 步客观验证循环：
...
`xirang-reviewer` skill SHALL 定义以下严重性分类和证据标准：
...
### Requirement: 三个验证维度
`xirang-reviewer` skill SHALL 覆盖四个验证维度及可选的 OPSX 对齐检查，并按 Check 的锚点类型分派判定模式：`Verifies`（普通 requirement）执行存在性判定，`Verifies ... REMOVED Requirement` 执行缺失性判定，`Preserves` 执行等价性判定。
...
- **WHEN** 在 Windows 上为 Claude Code 执行 `xirang init`
