...
Core workflows SHALL 使用同一 OPSX Semantic Model loading protocol：从唯一 Project Root 读取 project-level intent，沿 containment 获取 abstraction/refinement context，通过 `xirang arch query` 获取 stable elements、owned Specs 与 semantic relationships。Code locations SHALL 继续由 CodeGraph 或 ACE/`rg`/`read` 提供。
...
Shared query fragment SHALL 将 `xirang arch query <elementId> --relations --depth <n> --json` 定义为 hierarchy、contract 与 relation detail interface。Guidance SHALL 解释 FQN 与 stable elementId 的区别。
