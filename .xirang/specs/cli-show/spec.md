...
## Purpose
Define top-level `xirang show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `xirang show` without arguments
...
- **WHEN** executing `xirang show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `xirang show <item>` or `xirang change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `xirang show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `xirang change show`/`xirang spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `xirang show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `xirang show --type spec <item>`
...
- **WHEN** executing `xirang show <item> --json`
...
- **WHEN** showing a change with `xirang show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `xirang show <spec-id> --requirements`
