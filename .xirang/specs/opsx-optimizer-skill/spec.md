---
element: cap.ai.optimizer-skill
---
...
`xirang-optimizer` SHALL 是 Phase 2 finding-first 通用优化审查者，负责判断正确实现是否值得优化及应如何优化。它 MUST 使用 fresh context 自主读取项目证据，MUST NOT 修改任何文件，MUST NOT 改变 specs、公开契约或领域语义，MUST NOT 配置默认 `model`。
