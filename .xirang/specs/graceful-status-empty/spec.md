...
- **WHEN** user runs `xirang status` without `--change` and no change directories exist under `.xirang/changes/`
- **THEN** the CLI prints `No active changes. Create one with: xirang new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `xirang status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `xirang status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `xirang status --change non-existent`
- **THEN** the CLI throws an error with message `Change 'non-existent' not found`

#### Scenario: Other commands unaffected
- **WHEN** user runs `xirang show` or `xirang instructions` without `--change` and no changes exist
