...
Snack SHALL 运行 full change validation，并审阅统一 effective semantic diff。出现 ERROR/WARNING 时 SHALL 修复一轮并复检一次；validation 无 ERROR 后 SHALL 运行 `xirang diff --change "<name>" --write`，且 MUST NOT 生成或写入 Scenario labels。
