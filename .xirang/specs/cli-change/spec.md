---
element: cap.cli.change
---

# cli-change Specification

## Purpose
Define `xirang change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `xirang change show update-error --json`
...
- **WHEN** executing `xirang change list`
- **THEN** scan the .xirang/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `xirang change show update-error --requirements-only`
...
- **WHEN** executing `xirang change validate update-error`
...
- **WHEN** executing `xirang list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'xirang list' is deprecated. Use 'xirang change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `xirang list --all`
...
- **WHEN** executing `xirang change show` without arguments
...
- **WHEN** executing `xirang change show` without a change name
...
- **WHEN** executing `xirang change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `XIRANG_INTERACTIVE=0`
- **WHEN** executing `xirang change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
