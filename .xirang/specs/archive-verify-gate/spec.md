...
系统 SHALL 在归档任意活动 change 之前取得一份 fresh 的 full verify 结果，并通过 CLI 工具（`xirang verify status` 或等价的 freshness 检查）执行门禁校验。该结果 SHALL 包含 optimization 字段，且其终局状态必须与 verify 的 checkpoint 生命周期语义一致。

#### Scenario: Archive 通过 CLI 校验 verify gate

- **WHEN** agent 准备归档某个 change
- **AND** agent 调用 `xirang archive <change-name>`（或模板中等价的 freshness 预检查）
...
系统 SHALL 在缺失或 stale verify result 触发 full verify rerun 时，复用与 `/xirang:verify` 一致的 subagent-orchestrated execution model。

#### Scenario: archive rerun 复用 subagent orchestration

- **WHEN** archive 检测到 `.verify-result.json` missing 或 stale
- **THEN** archive SHALL 执行与 `/xirang:verify` 相同的 subagent-orchestrated full verify contract
