...
## Purpose
Define `xirang feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `xirang feedback` command that creates a GitHub Issue in the xirang repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.
...
- **WHEN** user executes `xirang feedback "Title here" --body "Detailed description..."`
...
- **WHEN** user has disabled telemetry via `XIRANG_TELEMETRY=0`
- **AND** user runs `xirang feedback "message"`
...
- **AND** submits via `xirang feedback` command on user confirmation
