---
element: cap.cli.spec
---

# cli-spec Specification

## Purpose
Define `xirang spec` command behavior for listing, showing, and validating source-of-truth specifications.
## Requirements
...
- **WHEN** executing `xirang spec show` without arguments
...
- **WHEN** executing `xirang spec show` without a spec-id
...
- **WHEN** executing `xirang spec validate` without arguments
- **THEN** display an interactive list of available specs
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `XIRANG_INTERACTIVE=0`
- **WHEN** executing `xirang spec validate` without a spec-id
- **THEN** do not prompt interactively
- **AND** print the existing error message for missing spec-id
- **AND** set non-zero exit code
