...
The `xirang view` command provides a comprehensive dashboard view of the OPSX project state, displaying specifications, changes, and progress metrics in a unified, visually appealing format to help developers quickly understand project status.
## Requirements
...
- **THEN** 系统 SHALL 从当前目录或其祖先寻找最近的 `.xirang/` 项目根
...
#### Scenario: 项目根发现
- **WHEN** 用户从 `.xirang/changes/some-change/` 子目录运行 `xirang view`
...
- **WHEN** 项目包含一个或多个 `.xirang/changes/<name>/` active changes
