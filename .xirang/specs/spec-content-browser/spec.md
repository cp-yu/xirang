...
服务端 SHALL 根据当前 computed OPSX Semantic Model 的 stable `elementId` 与派生 registry 授权 `.xirang/specs/**/*.md` 读取。请求 path 必须属于该 element 的 registry entries，并通过 project-relative、extension 与 realpath containment 校验。
...
#### Scenario: 路径安全
- **WHEN** path 为 absolute、包含 `..`、包含 backslash、不是 `.md` 或 realpath 逃逸 `.xirang/specs/`
...
#### Scenario: 符号链接逃逸
- **WHEN** `.xirang/specs/` 下 symlink 的 realpath 指向目录外
- **THEN** server SHALL 拒绝读取
...
- **AND** Spec SHALL 按需从 `.xirang/specs/` 读取
...
- **AND** identity 与 counts SHALL 与 `xirang diff --json` 一致
