...
## Purpose
定义 `/xirang:verify` 如何将关键诊断结果回写到 change 工件中，使后续实现和归档阶段能够基于最新验证结论继续推进。
## Requirements
...
- **AND** 持久化逻辑由 verify CLI 工具（`xirang verify phase1 --input --json`）处理，reviewer subagent 的 writeBackPlan 通过 reviewer.ts Output Contract 定义
