...
- **THEN** manifest entry SHALL 使用 workflow ID `build`、skill name `xirang-build` 和 skill directory `xirang-build`
...
Setup 和 update SHALL 使用 shared artifact sync engine 生成 `xirang-build` 及相关 managed artifacts，并 SHALL NOT 生成第二套 bootstrap command surface。
