...
Dynamic completion SHALL 继续为 change IDs 与 Spec IDs 提供 project-aware suggestions，并 SHALL NOT 从 `.xirang/candidate/` 或 `.xirang/history/` 推断 formal identifiers。

#### Scenario: Formal ID completion
- **WHEN** command 需要 change 或 Spec identifier
- **THEN** provider SHALL 从 `.xirang/changes/` 或 `.xirang/specs/` 读取 suggestions
...
- **WHEN** user executes `xirang completion install fish`
...
- **WHEN** user executes `xirang completion install powershell`
...
- **WHEN** user executes `xirang completion install` without specifying a shell
...
- **WHEN** user executes `xirang completion uninstall` without specifying a shell
...
- **AND** display usage hint: "Usage: xirang completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `xirang completion generate zsh > /path/to/_opsx`
