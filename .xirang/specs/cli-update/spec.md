...
- **WHEN** updating files
- **THEN** completely replace `xirang/AGENTS.md` with the latest template
...
- **THEN** replace `xirang/AGENTS.md` with the latest template
...
`xirang update` SHALL 只在包含 `.xirang/` 的项目内运行。
...
- **WHEN** managed `xirang-bootstrap-arch` skill 存在
...
`xirang update` SHALL 在现有 OPSX 项目中迁移项目配置默认值，通过在 `.xirang/config.yaml` 或 `.xirang/config.yml` 中物化缺失的功能性默认值（不覆盖用户值），并移除过时的 `git.merge.messageFrom`、`git.autoCommit`、`git.archive.commitMessage.convention` 和 `git.merge.commitMessage.convention` 字段。

#### Scenario: 配置缺失时创建

- **WHEN** 项目包含 `.xirang/` 目录
- **AND** `.xirang/config.yaml` 和 `.xirang/config.yml` 均不存在
...
- **WHEN** `.xirang/config.yaml` 包含 `optimization.enabled: false`
...
- **WHEN** `.xirang/config.yaml` 包含 `git.merge.messageFrom`、`git.autoCommit`、`git.archive.commitMessage.convention` 或 `git.merge.commitMessage.convention` 中的任何一个
- **AND** 用户运行 `xirang update`
...
- **AND** `.xirang/config.yml` 存在且包含有效 YAML 对象内容
- **AND** 用户运行 `xirang update`
...
### Requirement: 固定工作流更新
`xirang update` SHALL 刷新全部六个 fixed workflow skills，并 SHALL NOT 读取 retired workflow-selection settings。
...
当 update 或 setup 发现 `.xirang/bootstrap/`、`.xirang/bootstrap-history/` 或 `.xirang/migration-candidate/` 时，SHALL 在用户确认后将其移动到显式 `.xirang/history/legacy-<timestamp>/` entry。
...
- **AND** the default directory name `xirang` SHALL be used
