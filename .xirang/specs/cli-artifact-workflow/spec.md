---
element: cap.cli.artifact-workflow
---
...
- **WHEN** user runs `xirang status --change <id>`
...
- **WHEN** user runs `xirang status --change <id> --json`
...
- **WHEN** user runs `xirang status --change <id>` on a change with no artifacts
...
The workflow SHALL use `xirang status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `xirang status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `xirang instructions <artifact> --change <id>`
...
- **WHEN** user runs `xirang instructions <artifact> --change <id> --json`
...
- **AND** `Verifies:` SHALL NOT use `.xirang/specs/...`, project-root-relative paths, absolute paths, archived/main spec paths, or backslash-separated paths
...
- **WHEN** user runs `xirang instructions apply --change <id> --json`
...
- **AND** tests SHALL cover rejection of `.xirang/specs/...`, absolute paths, parent traversal, and backslash-separated paths
...
- **THEN** 该字段 SHALL 使用 `.xirang/specs/<capability>/spec.md` 形式的项目根相对 POSIX 路径
...
- **WHEN** user runs `xirang templates`
...
- **WHEN** user runs `xirang templates --json`
...
- **WHEN** user runs `xirang new change add-feature`
- **THEN** the system creates `.xirang/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `xirang new change "Add Feature"` with invalid name
...
The system SHALL generate schema-aware apply instructions via `xirang instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `xirang instructions apply --change <id>`
...
- **WHEN** user runs `xirang artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `xirang artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `xirang artifact-experimental-setup --tool unknown-tool`
...
`xirang instructions <artifact> --change <id>` SHALL 返回 resolved built-in Schema 中该 artifact 的结构化 file definition，使 Agent 在 authoring 前理解文件的 durable semantics、compilation role、content boundary、write policy 与 validation commands。
...
`xirang instructions <artifact>` SHALL 在 text 与 JSON modes 投影同一 artifact current state 语义，并 SHALL 保持 definition-first 展示顺序。Renderer SHALL 将 definition、dependencies/current state、instruction 与 template 作为独立 sections，MUST NOT 将文件内容嵌入 current state。
...
`xirang instructions apply` SHALL 在 Apply prerequisites 未满足时返回 schema-aware prerequisite workflow，而 MUST NOT 指示 Apply workflow 创建自身缺失的输入。Spec-driven schema SHALL hand off 到 Propose；Bootstrap schema SHALL hand off 到 Bootstrap。
