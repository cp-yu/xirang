...
`xirang-apply-change` SHALL 从生成 reference 读取 finding-driven Phase 2 协议。Phase 2 SHALL 创建非空 baseline commit，调用 fresh optimizer 生成或 reconciliation 全部 findings，只实现最新排序中的首个 actionable finding，调用 CLI 记录 pre-implementation hashes，由 master 按 TDD 编码，再由 fresh reviewer 验证。
