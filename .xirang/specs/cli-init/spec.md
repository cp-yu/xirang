...
The `xirang init` command SHALL create a complete OPSX directory structure in any project, enabling immediate adoption of OPSX conventions with support for multiple AI coding assistants.
## Requirements
### Requirement: OPSX Setup SHALL 创建可用的 formal skeleton
`xirang setup` SHALL 创建或保留 `.xirang` durable core、project configuration、versioned formal LikeC4 skeleton 和 empty Specs directory，且不得声称 Project Build 已完成。

#### Scenario: 新项目 setup
- **WHEN** project 中不存在 `.xirang/` workspace
- **THEN** setup SHALL 创建 `.xirang/config.yaml`、`.xirang/architecture/`、`.xirang/specs/`、`.xirang/changes/` 和 `.xirang/references/`
...
- **AND** SHALL NOT 生成 `xirang-bootstrap-arch`
