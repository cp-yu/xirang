...
## Command Syntax
```bash
xirang archive [change-name] [--yes|-y]
```
...
- **THEN** SHALL 运行等价于 `xirang diff --change <name> --write` 的 final report generation
...
- **THEN** handle the following conditions:
  - Missing .xirang/changes/ directory
  - Change not found
...
- **WHEN** change 包含尚未通过 `xirang sync` 合并到 main spec 的 delta
- **AND** 未使用 `--no-sync`
- **THEN** archive 命令 SHALL 输出错误消息指明 pending delta
- **AND** SHALL 提示用户执行 `xirang sync <change-name>` 后重试
- **AND** SHALL 终止归档操作

#### Scenario: delta 已全部合并时通过检查

- **WHEN** change 的所有 delta（spec + OPSX）已通过 `xirang sync` 合并到 main spec
- **AND** 未使用 `--no-sync`
...
- **WHEN** 执行 `xirang archive <change> --no-sync`
- **AND** 非 `--yes` 模式
...
- **WHEN** 执行 `xirang archive <change> --no-sync --yes`
