---
element: cap.framework.conventions
---
...
#### Scenario: 初始化项目结构
- **WHEN** `xirang setup` 初始化项目
- **THEN** SHALL 创建 formal `architecture/`、`specs/`、`changes/`、`references/` 与 `config.yaml`
- **AND** Project Build SHALL 只在 `.xirang/candidate/` 编写待提升的 `build.md`、Architecture 和 Specs
- **AND** successful promotion SHALL 在 `.xirang/history/builds/` 保存 previous formal source
...
- **AND** MAY 使用 `xirang diff --change <name>` 查看完整 text diff
- **AND** MAY 使用 `xirang diff --change <name> --write` 生成 `effective-change.md`
- **AND** MAY 在 `xirang view` 选择该 active change 查看 Specs 与 Architecture diff
...
- **AND** SHALL NOT 保留 `xirang init` alias
...
- **WHEN** 用户请求 `xirang bootstrap` 或 `xirang migrate`
...
6. **Update**: Delta specs are merged into main `specs/`; `architecture-delta.c4` is merged into `.xirang/architecture/`
