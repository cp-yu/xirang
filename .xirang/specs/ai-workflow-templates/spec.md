...
Verify/apply/archive 三个模板中 delegate to reviewer subagent 的步骤 SHALL delegate to 对应的 `xirang-reviewer` agent。Delegate to optimizer subagent 的步骤 SHALL delegate to 对应的 `xirang-optimizer` agent。

模板中的 delegation 指令 SHALL 使用 agent 名引用（`xirang-reviewer`、`xirang-optimizer`、`opsx-impact-sweeper`）。Subagent 由 agent 文件承载，模板 SHALL 引用 `.xirang/references/` 路径下的 reference 文件获取步骤详情。
