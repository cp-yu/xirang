...
The `xirang list` command SHALL provide developers with a quick overview of all active changes in the project, showing their names and task completion status.
## Requirements
### Requirement: Command Execution
The command SHALL scan and analyze either active changes or specs based on the selected mode.

#### Scenario: Scanning for changes (default)
- **WHEN** `xirang list` is executed without flags
- **THEN** scan the `.xirang/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs
- **WHEN** `xirang list --specs` is executed
- **THEN** scan the `.xirang/specs/` directory for capabilities
...
- **WHEN** `.xirang/changes/` directory doesn't exist
- **THEN** display error: "No OPSX changes directory found. Run 'xirang init' first."
...
`xirang list --specs --json` SHALL 返回 Spec identity、requirements 与 singular element binding。Requirement names SHALL 从 formal `### Requirement:` headers 提取。

#### Scenario: JSON output includes element and requirements
- **WHEN** 用户运行 `xirang list --specs --json`
