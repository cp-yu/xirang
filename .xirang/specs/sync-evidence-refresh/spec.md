---
element: cap.sync.evidence-refresh
---

# sync-evidence-refresh Specification

## Purpose
Define the reviewed Finding-Driven Optimization Gate contract for sync 完成后刷新 evidence fingerprint.
...
- 所有 architecture manifest writes：`prepared.manifest[]` 中 scope 为 `architecture` 的 project-relative paths
...
- **AND** entries 中包含一个实际 architecture manifest path
- **AND** `prepared.architecture` 非 null（sync 将写入 architecture modules）
- **WHEN** `applyPreparedChangeSync` 完成 architecture 和 Spec 写入
- **THEN** 系统 SHALL 读取该 synced architecture file 的当前内容
...
- **AND** sync 改写一个 recorded architecture file
...
- **GIVEN** evidenceFingerprintEntries 中包含 `.xirang/changes/<name>/specs/foo/spec.md`
...
- **WHEN** 外部进程再次修改该 architecture file
- **THEN** 后续 `checkFreshness` SHALL 检测到 STALE
- **AND** details SHALL 列出该 normalized architecture path 为变更文件
