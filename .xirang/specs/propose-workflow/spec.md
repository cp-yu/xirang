...
- **THEN** SHALL 依赖 `xirang instructions specs --change "<name>" --json` 返回的 definition
...
- **AND** SHALL 运行 `xirang diff --change "<name>" --write`
...
- **AND** validation 无 ERROR 后 SHALL 运行 `xirang diff --change "<name>" --write`
...
- **WHEN** `xirang diff` 显示未授权或遗漏的 operation
