...
Define the expected behavior for the `/xirang:archive` skill, including readiness checks, spec sync prompting, archive execution, and user-facing output.
## Requirements
### Requirement: OPSX Archive Skill

The system SHALL provide an `/xirang:archive` skill that archives completed changes in the experimental workflow.
...
- **THEN** the skill SHALL 复用与 `/xirang:verify` 相同的 subagent-orchestrated verify 模板骨架
...
- **THEN** the skill SHALL 继续执行与 `/xirang:verify` 完全一致的 Phase 2 optimization contract
...
The skill SHALL handle sync inline during archive instead of requiring a separate `/xirang:sync` surface.
...
- **AND** SHALL NOT require an installed separate `/xirang:sync` skill
...
`opsx-archive-skill` SHALL 将 boundary commit、archive commit 与 merge summary commit 的 message 格式说明作为 `.xirang/references/` 下的受管 reference 文件提供，并要求 agent 在归档后的 git 流程中按 `git.commitMessage.*` 覆盖路由读取。
...
- **WHEN** 生成 `xirang-archive-change` skill
- **THEN** 主 `SKILL.md` SHALL 要求 agent 在创建 OPSX/docs 归档制品 commit 前读取 `git.commitMessage.archive` 指向的模板，未配置时读取 `.xirang/references/xirang-archive-commit-message.md`
...
`xirang-archive-change` skill SHALL 在用户显式要求时允许 agent 向 archive CLI 传递 `--no-verify` 标志。skill SHALL 优先引导 agent 走标准 verify gate 流程，但当用户明确表示要跳过 verify 时，agent SHALL 直接传递 `--no-verify`，由 CLI 自行执行二次确认。
...
`/xirang:archive` skill SHALL 根据 `xirang verify status <change-name> --json` 返回的 `freshness.status` 路由 verify 执行。
