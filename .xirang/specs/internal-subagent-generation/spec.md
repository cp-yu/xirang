...
系统 SHALL 在 `src/core/shared/subagent-generation.ts` 中提供 `INTERNAL_SUBAGENT_TEMPLATES` 常量，显式注册三个 internal subagent：`xirang-reviewer`、`xirang-optimizer`、`opsx-impact-sweeper`。该常量 SHALL NOT 包含 `opsx-implementer`，且 SHALL NOT 通过目录扫描、glob filtering 或 regex inference 推断成员。
...
- **THEN** SHALL 包含 `xirang-reviewer`、`xirang-optimizer`、`opsx-impact-sweeper` 三个显式条目
...
- **THEN** 列表 SHALL NOT 包含 `xirang-reviewer`、`xirang-optimizer` 或 `opsx-impact-sweeper`
...
Renderer SHALL NOT 复用 workflow invocation transform 管线（如 `/xirang:<slug>` 替换）。若需要 transform context，SHALL 使用独立的 `artifactType: 'subagent'`。
...
- **AND** 对 `xirang-reviewer`、`xirang-optimizer`、`opsx-impact-sweeper`，`description` SHALL 追加 Pi-only 提示：`Pi callers: run foreground and omit timeoutMs/maxRuntimeMs.`
...
- **WHEN** 在 Windows 上为 Claude Code 写入 `xirang-reviewer` subagent artifact
...
- **WHEN** 为 Codex 写入 `xirang-optimizer` subagent artifact
...
- **WHEN** 渲染 `xirang-reviewer` 的 Claude / Pi / OpenCode / Codex artifact
...
- **THEN** 声明的工具权限 SHALL 与 `.xirang/specs/subagent-self-read/spec.md` 中对应角色的权限模型一致
