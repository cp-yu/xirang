...
系统 SHALL 扫描 `.xirang/specs/*/spec.md` 的 singular `element` frontmatter，构建 `elementToSpecs` 与 `specToElement` 派生 registry。一个 element MAY 对应多个 Specs；每个 Spec SHALL 最多产生一个 owner entry。
...
- **THEN** SHALL 使用 Node.js path API 定位 `.xirang/specs`
