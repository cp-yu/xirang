...
> OPSX should have one default path, one escalation path, and fewer explicit concepts shown to the user unless the system actually needs a decision from them.
...
- "OPSX figures out whether this stays local or needs to expand"
- "If it expands, it carries me forward instead of making me restart"
- "I only see advanced concepts when OPSX needs a real decision from me"
...
2. Run `/xirang:explore` or `/xirang:propose`
3. OPSX plans locally unless it has a strong reason not to
...
> This work spans multiple owned areas strongly enough that OPSX needs to upgrade it into a coordinated initiative.
...
- "OPSX is upgrading this into a coordinated initiative"
...
### Concepts OPSX should keep implicit until needed
...
### Concepts OPSX should only show when a real decision is needed
...
If OPSX realizes the work is no longer local, it should escalate in one motion.
...
If escalation is needed, OPSX should do as much as possible automatically:
...
OPSX can upgrade this into a coordinated initiative.

Suggested next step:
- create a coordination workspace at ~/work/opsx-workspaces/add-3ds
...
When OPSX has to ask questions, it should ask the smallest useful set.
...
Only ask if OPSX has strong evidence of a cross-boundary contract.
...
To keep the UX intuitive, OPSX should aggressively choose defaults.
...
OPSX should infer affected areas from:
...
If an existing shared contract owner already exists, OPSX should suggest it instead of asking an abstract ownership question.
...
OPSX can start with a local coordination workspace and later offer:
...
2. OPSX upgrades to a coordinated initiative if needed
3. When the work becomes collaborative, OPSX offers to make it team-shared
...
OPSX should avoid asking users to classify work in abstract ways if it can infer a reasonable default.
...
By default, OPSX should feel like:
...
- progressive disclosure: only show advanced concepts when OPSX needs a real decision

If OPSX does this well, the same system can feel intuitive for:
