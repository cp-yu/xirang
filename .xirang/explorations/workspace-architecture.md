...
```
xirang profile set core|extended
xirang profile install <workflow>
xirang profile uninstall <workflow>
xirang profile list
xirang profile show
xirang config set delivery skills|commands|both
xirang config get delivery
xirang config list
```
...
**After (simplified):**
```
xirang config profile          # interactive picker (delivery + workflows)
xirang config profile core     # preset shortcut
xirang config profile extended # preset shortcut
```
...
- **Global** = user's defaults (`~/.config/opsx/`)
- **Project** = repo-level config (`.xirang/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `xirang init`
2. `xirang init` applies current profile to project
...
### Current OPSX Architecture

OPSX currently assumes:
- One `xirang/` per repo, always at root
...
├── services/
│   └── billing/
│       └── .xirang/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── .xirang/specs/checkout/spec.md
│   ├── ios/
│   │   └── .xirang/specs/checkout/spec.md
│   └── android/
│       └── .xirang/specs/checkout/spec.md
└── xirang/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `xirang/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `xirang/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `xirang/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `xirang/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
~/work/
├── billing-service/
│   └── .xirang/specs/billing/
├── web-client/
│   └── .xirang/specs/checkout/
├── ios-client/
│   └── .xirang/specs/checkout/
...
2. **Where do cross-repo changes live?**
   ...
   - In `~/.config/opsx/workspaces/my-platform/changes/`?
...
> **A workspace is a collection of OPSX roots that can be operated on together.**

```yaml
# ~/.config/opsx/workspaces.yaml (or similar)
workspaces:
  my-platform:
    roots:
...
- `.xirang/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `.xirang/specs/_shared/checkout-contract/spec.md`
- Or `.xirang/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single OPSX root (one `xirang/` directory) |
...
### 4. Does OPSX need to understand dependencies?

If `checkout-web` depends on `checkout-contract`:
- Should OPSX know this relationship?
...
**Ambitious:**
> OPSX automatically understands your repo structure, detects cross-cutting specs, and helps you create changes that flow to the right places.

**Simpler:**
> You organize specs however you want. OPSX just works.
...
### What is a spec in OPSX?

For OPSX, a spec should be treated as a **verifiable behavior contract at a boundary**:
...
OPSX is often agent-authored from human exploration. To make that reliable:
...
2. `.xirang/specs/opsx-conventions/spec.md` for normative spec conventions
3. `.xirang/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules

---

## Part 10: Design Decisions (April 2026)

After evaluating the models above against real multi-repo use cases (see [#725](https://github.com/Fission-AI/OPSX/issues/725)), we converged on the following design direction.
...
- **Each repo keeps its own canonical `xirang/`** — no change to the fundamental storage model.
...
For larger multi-team work, repo-centric planning is the wrong primary abstraction. Teams and repos are many-to-many facets over the same work. OPSX should treat the **initiative / plan** as the first-class planning object, then link repo-local changes to it.
...
```yaml
# billing-service/.xirang/changes/add-3ds/.xirang.yaml
schema: spec-driven
created: 2026-04-12
initiative: add-3ds
links:
...
2. **Local OPSX repo registry** — a persistent mapping in `~/.config/opsx/` or `~/.local/share/opsx/` (see `src/core/global-config.ts`).
...
```yaml
# web-client/.xirang/specs/checkout/spec.md frontmatter
references:
...
This avoids accidentally committing OPSX to a full dependency graph system before the use cases justify it.
...
|---------|----------|------------|
| **Spec organization** | Nested specs inside one `xirang/` (Model B) | Each repo has its own `xirang/` |
...
|----------|--------|-------|
| Profile UX | Decided | `xirang config profile` with presets |
