...
This document describes the exact user experience for OPSX across:
...
- How OPSX decides which specs to read
- How OPSX behaves when roots are missing, ambiguous, or partially available
...
- OPSX reads only the relevant specs
- OPSX stores each artifact in the owning place
- If the work spans multiple owning places, OPSX creates a neutral coordination workspace
...
OPSX should model them explicitly as:
...
If there is no owner yet, the shared behavior can live temporarily as an initiative note during exploration, but OPSX should not promote it into a canonical spec until ownership is explicit.
...
Users should still recognize the existing OPSX workflow:
...
The new behavior is that OPSX may ask:
...
Instead, OPSX should create a neutral coordination workspace for the initiative.
...
OPSX may surface them to users and agents, but should not require them to resolve or validate across roots in the first version.

### 6. Shared contracts need explicit ownership

When the user describes behavior that spans multiple boundaries, OPSX should help them decide whether they are creating:

- a local spec
- a shared contract
- an initiative note that is not yet canonical

If the work is a real long-lived contract, OPSX should prompt for a canonical owner rather than burying it in whichever repo the user happened to be standing in.

### 7. Do not force "shared ownership" as implicit co-ownership

OPSX should help teams create **shared contract ownership**, not ambiguous co-ownership.
...
When a user starts in `web-client` and describes a checkout behavior spanning web, iOS, Android, and backend, OPSX should treat the current cwd as a clue about likely consumers, not as proof that `web-client` should own the shared contract.
...
A single OPSX root where canonical specs and changes can live.

In practice a project may be:

- a repo root with `xirang/`
- a monorepo root with one `xirang/`
- a nested owned scope if OPSX later supports nested roots
...
```text
monorepo/
  apps/web/opsx.scope.yaml
  services/billing/opsx.scope.yaml
  packages/contracts/opsx.scope.yaml
```
...
OPSX needs to answer two separate questions:
...
OPSX discovers candidate scopes from:
...
OPSX resolves these to local paths using this order:

1. explicit path passed for this run
2. saved local OPSX registry
...
For team use, OPSX should separate:
...
OPSX should communicate this clearly:
...
OPSX should help teams choose the owner that can most credibly do all of the following:
...
When OPSX detects a likely shared contract, it should ask or infer from these questions:
...
OPSX should default to that existing owner.
...
OPSX should suggest that existing domain owner if there is no dedicated shared area.
...
OPSX should help create a new shared contract scope.
...
OPSX should allow the behavior to remain in initiative notes temporarily and explicitly mark it as non-canonical until ownership is chosen.

### What OPSX should suggest by default
...
When the user describes a likely cross-boundary behavior, OPSX should treat this as a first-class planning moment.
...
During `/xirang:propose` or `/xirang:explore`, OPSX should ask something like:
...
If the user chooses "create a new shared contract scope," OPSX should guide them.
...
- .xirang/specs/contracts/checkout
- .xirang/specs/shared/checkout
...
OPSX should avoid auto-creating a brand-new repo. It can scaffold the plan and record the decision, but repo creation may be organizationally sensitive and usually belongs outside the CLI.

### Temporary initiative-only mode

If ownership is unclear, OPSX should support:
...
4. Let OPSX determine whether this is:
   - local only
   - multi-scope in one root
   - cross-root coordination
5. OPSX creates artifacts in the right places
6. OPSX tells the user where to continue planning and where to implement
...
OPSX should support three agent capability levels.
...
- OPSX tells the user exactly which roots to attach
...
- User opens the coordination workspace
- OPSX writes agent-readable absolute paths into the workspace instructions
...
- OPSX guides the user into repo-local implementation sessions afterward
- `/xirang:apply` runs in each repo separately

This must be treated as a first-class case, not a fallback afterthought.

---

## Current OPSX Flow To Preserve

Today the flow is:

1. User enters a repo
2. User runs `xirang init`
3. User opens an agent in that repo
4. User runs `/xirang:explore` or `/xirang:propose`
5. OPSX stores changes in `.xirang/changes/<change>/`
6. `/xirang:apply` implements within that repo

That should remain the exact feel for single-repo work.

The only extension is:

- if OPSX detects a cross-root initiative, it explicitly upgrades the workflow into a coordination workspace flow

---

## Journey 1: Single Repo, Standard OPSX Project
...
```text
Created .xirang/changes/add-dark-mode/
Using scope: repo
...
Ready for implementation with /xirang:apply
```
...
OPSX archives the change in this repo, as today.
...
- User runs from subdirectory: OPSX should either walk up or clearly tell them to run from repo root
...
```text
/xirang:propose add-invoice-filtering
```

OPSX detects this is still one project root, but there are multiple candidate scopes.

If the user’s request clearly mentions one area, OPSX may infer:
...
1. create a single monorepo-local change under `.xirang/changes/add-invoice-filtering/`
...
```text
/xirang:propose add-3ds-checkout
```

OPSX detects likely affected scopes:
...
```text
platform/.xirang/changes/add-3ds-checkout/
```
...
OPSX ignores unrelated specs by default.
...
```text
/xirang:propose add-checkout-loading-state
```

OPSX detects this is limited to `apps/web`.
...
At this point OPSX must make a product decision.
...
If the user chooses the coordination flow, OPSX creates:
...
At this point OPSX should not silently create one cross-repo change inside `web-client`.
...
### Expected OPSX behavior

OPSX interrupts the default single-repo flow and says:
...
For cross-repo work, OPSX recommends creating a coordination workspace.
...
If the user agrees, OPSX creates the workspace.
...
```bash
xirang workspace create add-3ds --at ~/work/opsx-workspaces/add-3ds
```
...
OPSX now resolves local paths for:
...
OPSX then tells the user:

```text
Next step:
1. Open your coding agent in ~/work/opsx-workspaces/add-3ds
...
OPSX has generated workspace instructions at:
.opsx-workspace/agents/claude.md
```
...
Or OPSX may have already scaffolded the initiative and tell the agent to continue it.
...
- `contracts/.xirang/changes/add-3ds-contract/`
- `billing-service/.xirang/changes/add-3ds-billing/`
- `web-client/.xirang/changes/add-3ds-web/`
- `ios-client/.xirang/changes/add-3ds-ios/`
...
OPSX resolves local clones and writes the workspace files.

### Agent setup

The user opens the coordination workspace in their agent.

OPSX-generated agent instructions contain:
...
OPSX should support that directly.
...
Each teammate keeps local path mappings outside the shared repo, for example in OPSX local config/data:
...
2. runs something like `xirang workspace doctor` or `xirang workspace sync`
...
OPSX generates agent instructions using:
...
OPSX should support the initiative being created in a shared coordination repo owned by the sponsoring or driving team.
...
When another team member opens the shared coordination workspace, OPSX should make this explicit:
...
OPSX should still allow planning.
...
OPSX should never blindly read all specs from all roots.
...
```text
Related references:
- github.com/Fission-AI/contracts: .xirang/specs/checkout/spec.md
- github.com/Fission-AI/web-client: .xirang/specs/checkout/web/spec.md
```
...
When a selected change involves a shared contract, OPSX should prefer this read order:
...
If a cross-boundary behavior has not yet been assigned a canonical owner, OPSX should store it as initiative-level draft material rather than pretending it is already a canonical spec.
...
## Journey 10: `/xirang:apply` In Cross-Root Work
...
Next suggested step:
- open /Users/me/work/contracts and run /xirang:apply add-3ds-contract
```
...
OPSX detects this likely requires a shared contract.
...
If the user selects an existing contracts repo, OPSX:
...
If the user keeps it initiative-only, OPSX:
...
OPSX checks linked repo changes:
...
During planning, OPSX asks for scope selection when needed.
...
1. each repo independently runs `xirang init`
...
When OPSX creates a coordination workspace, it should generate agent-facing files.
...
- OPSX recommends creating a coordination workspace
...
- `xirang workspace doctor` or equivalent relinks roots
...
If OPSX adopts these journeys, several design conclusions follow.

### 1. Multi-repo planning needs a first-class coordination workspace

Without this, OPSX has no honest answer to where the user should stand or where the cross-repo initiative should live.

### 2. Repo-local changes and canonical specs should stay in owners

Without this, OPSX creates duplicate or misleading sources of truth.

### 3. Large monorepos cannot be treated as always-simple single roots

OPSX must support both:
...
Cross-root planning only works reliably if OPSX writes deterministic workspace context for the agent.

### 6. References should stay informational in v1

If OPSX validates cross-root references by default, it has effectively shipped a dependency graph system.
...
OPSX should help users choose among:
...
To support teams and orgs cleanly, OPSX should distinguish:
...
1. Should a coordination workspace always be user-visible on disk, or may it optionally live in global OPSX data directories?
...
3. Should OPSX support nested `xirang/` roots, or keep one root and model scopes separately?
