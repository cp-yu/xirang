...
- [ ] C4 验证 Definition depth 语义和 CLI override
...
- [ ] C7 验证旧 query navigation interface 已删除
  - Verifies: `elements/arch-query.md` / REMOVED Requirement "arch query SHALL 支持 --relations 选项"
  - Command: `pnpm vitest run test/commands/arch-query.test.ts test/integration/arch-command.test.ts`
  - Expect: `--relations` 与 `--depth` 均产生明确 unknown-option failure，result schema 不含 `children/refinement/relatedElements/relations`。

- [ ] C8 验证 query 错误原子性和 JSON schema
...
- [ ] C9 验证 identity-only impact 保留 traversal 能力
...
- [ ] C11 验证旧 workflow query navigation 调用不存在
...
- [ ] C12 验证 Explore 影响链和 task recovery
