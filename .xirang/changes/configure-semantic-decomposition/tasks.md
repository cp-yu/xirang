...
- [ ] C5 验证 update missing-only migration
...
- [ ] C6 验证 Build 结构形成与 fail-closed 行为
...
- [ ] C7 验证 Definition Framing 保持确认协议
...
- custom skill 保持用户自有，不进入 managed workflow manifest。

#### Checks

- [ ] C11 验证本仓库 normalized config 选择 custom skill
...
**Files**:
- Modify: `.pi/skills/`
- Test: `test/core/templates/skill-template-length-validation.test.ts`
...
- [ ] C13 验证共享生成、非结构排除与长度限制
  ...
  - Command: `pnpm exec vitest run test/core/templates/skill-template-length-validation.test.ts test/core/templates/skill-templates-parity.test.ts test/core/templates/semantic-model-consistency.test.ts`
