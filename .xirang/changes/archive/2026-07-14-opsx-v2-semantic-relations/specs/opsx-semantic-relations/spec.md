...
OPSX SHALL 使用 `schema_version: 2`，正式模型 SHALL 仅由 `openspec/project.opsx.yaml` 与 `openspec/project.opsx.relations.yaml` 组成。系统 MUST NOT 要求、生成或读取 `openspec/project.opsx.code-map.yaml`。
...
- **AND** 错误 SHALL 指向 `openspec help authoring project.opsx.relations.yaml` 与 bootstrap/refresh 重建路径
