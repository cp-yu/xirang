...
### Requirement: 降级为 WARNING 的补充条件

当实现可能存在但 reviewer 缺乏验证工具（如运行时行为、外部 API 集成），或设计决策偏离已在代码注释中显式说明理由时，reviewer SHALL 判定为 WARNING 而非 CRITICAL；不确定时仍 SHALL 升级到 CRITICAL。

#### Scenario: 设计决策偏离已注释说明理由
- **WHEN** design.md 决策与代码实现不同
- **AND** 代码注释显式说明偏离理由
- **THEN** reviewer SHALL 判定为 WARNING（而非 CRITICAL）
- **AND** SHALL 在 summary 中注明注释位置

#### Scenario: 缺乏验证工具时降级为 WARNING
- **WHEN** 实现可能存在但 reviewer 缺乏验证工具（如运行时行为、外部 API 集成）
- **THEN** reviewer SHALL 判定为 WARNING（而非 CRITICAL）
- **AND** SHALL 在 summary 中注明证据缺口
### Requirement: 三个验证维度
