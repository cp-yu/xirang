...
definition: Schema Resolution 定义项目本地 schema 解析行为：仅从 package 内置 schema 目录解析固定 `semantic-model` Schema，project-local 与 user data 目录不参与 lookup，不暴露内置 bootstrap schema，并保留 Schema binding 但合法值限于 `semantic-model`。
---

## Requirements

### Requirement: Built-in Schema resolution
系统 SHALL 仅从 package 内置 Schema directory 解析固定 `semantic-model` Schema。Project-local 与 user data Schema directories MUST NOT 参与 lookup、listing、template resolution 或 shadowing，且系统 SHALL NOT 暴露内置 `bootstrap` schema。

#### Scenario: 内置 Schema 解析
- **WHEN** 系统解析 `semantic-model`
- **THEN** SHALL 返回 package 内置 semantic-model Schema directory
...
- **THEN** SHALL 报告 Schema 不存在
- **AND** SHALL 列出固定合法 ID `semantic-model`
...
- **THEN** the system returns only package built-in schema names（`semantic-model`）
...
New change、project config、change metadata 与 workflow CLI SHALL 保留 Schema binding，但合法值 SHALL 限于 `semantic-model`。Resolution precedence SHALL 保持 explicit CLI option → change metadata → project config → `semantic-model` default。

#### Scenario: Change 持久化 binding
- **WHEN** 创建 change
- **THEN** change metadata SHALL 持久化 `semantic-model`
...
- **THEN** workflow context SHALL fail fast 并提供 `semantic-model` Required Corrections
