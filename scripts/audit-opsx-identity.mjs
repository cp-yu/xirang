...
const stalePatterns = [
  { name: 'legacy-product-identity', pattern: /open[\s_-]?spec/i },
  {
    ...
    pattern: /(?:^|[\s'"`(])opsx\/(?:architecture|bootstrap(?:-history)?|changes|references|specs|config\.yaml)/m,
  },
  {
    name: 'legacy-workspace-join',
    pattern: /path\.(?:join|resolve)\([^)]*['"]opsx['"]\s*,\s*['"](?:architecture|bootstrap(?:-history)?|changes|references|specs|config\.yaml)['"]/,
  },
  { name: 'removed-arch-preview', pattern: /arch preview/ },
]
const self = 'scripts/audit-opsx-identity.mjs'
...
console.log(`Scanned ${activeFiles.length} active OPSX files.`)
