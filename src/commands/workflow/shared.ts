...
/**
 * Returns the list of available change directory names under .xirang/changes/.
...
export async function getAvailableChanges(projectRoot: string): Promise<string[]> {
  const changesPath = path.join(projectRoot, XIRANG_DIR_NAME, 'changes');
  try {
...
export async function validateChangeExists(
  ...
  if (!changeName) {
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: xirang new change <name>');
    }
    throw new Error(
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: xirang new change <name>`
      );
    }
    throw new Error(
