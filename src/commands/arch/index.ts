...
export function registerArchCommand(program: Command): void {
  ...
  arch.command('query <element-id>')
    .description('Project one Element with its refinement and relationship context')
    .option('--relations', 'Include relationships and related Elements')
    .option('--depth <n>', 'Refinement and relationship depth')
    .option('--contract', 'Inline the complete Element Contract')
    .option('--json', 'Output canonical JSON')
    .action(async (id, options) => {
      try {
        const result = await queryArchitecture(process.cwd(), id, {
