 ...
 * - Opt-out via XIRANG_TELEMETRY=0 or DO_NOT_TRACK=1
...
const POSTHOG_HOST = 'https://edge.xirang.dev';
 ...
 * - XIRANG_TELEMETRY=0
 * - DO_NOT_TRACK=1
 * - XIRANG_INTERACTIVE=0
 * - CI is set (any CI environment)
 * - stdin is not a TTY
 */
export function isTelemetryEnabled(): boolean {
  // Check explicit opt-out
  if (process.env.XIRANG_TELEMETRY === '0') {
    return false;
  }
  ...
  if (process.env.XIRANG_INTERACTIVE === '0') {
    return false;
  }
 ...
 * @param version - The Xirang version
 */
