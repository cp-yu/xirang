...
export async function readProjectOpsx(
  ...
  if (raw?.schema_version !== OPSX_SCHEMA_VERSION) {
    throw new Error(
      `OPSX schema_version ${String(raw?.schema_version ?? 'missing')} is unsupported; use opsx-build with the source as user-approved evidence or a Candidate starting point.`
    );
  }
  ...
  if (!await FileSystemUtils.fileExists(relationsPath)) {
    throw new Error(`OPSX file not found: ${OPSX_PATHS.RELATIONS_FILE}. Run opsx setup or build the Project OPSX Candidate.`);
  }
  const rawRelations = parseYaml(await fs.readFile(relationsPath, 'utf-8'));
  if (rawRelations?.schema_version !== OPSX_SCHEMA_VERSION) {
    throw new Error(
      `OPSX relations schema_version ${String(rawRelations?.schema_version ?? 'missing')} is unsupported; use opsx-build with the source as user-approved evidence or a Candidate starting point.`
    );
  }
