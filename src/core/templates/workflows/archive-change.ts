...
const ARCHIVE_COMMIT_MESSAGE_SHARED_PATH = '.opsx/references/opsx-archive-commit-message.md';
const BOUNDARY_COMMIT_MESSAGE_SHARED_PATH = '.opsx/references/opsx-boundary-commit-message.md';
const MERGE_SUMMARY_MESSAGE_SHARED_PATH = '.opsx/references/opsx-merge-summary-message.md';
...
function buildAgentGitFlowStep(): string {
  return `   The agent continues the post-archive git flow. First handle the implementation boundary before OPSX/docs archive artifacts. If uncommitted real project implementation changes remain, create a normal implementation commit that contains only those changes. Then always create a semantic boundary commit with \`git commit --allow-empty\`; this boundary commit may be intentionally empty when the effective implementation diff is already carried by retained \`wip: opt-*\` checkpoint commits. If \`git.commitMessage.boundary\` is set, read that project-relative path; otherwise read the project-root file \`${BOUNDARY_COMMIT_MESSAGE_SHARED_PATH}\`. Use that template to build the boundary commit message and run \`git commit -F -\` for the boundary commit. If \`git.commitMessage.archive\` is set, read that project-relative path; otherwise read the project-root file \`${ARCHIVE_COMMIT_MESSAGE_SHARED_PATH}\`. Use that template before creating the OPSX/docs archive commit, add only archive/synced paths, and run \`git commit -F -\`. If a merge or squash commit message is needed, prepare it from the configured or built-in merge template. If \`git.commitMessage.merge\` is set, read that project-relative path; otherwise read the project-root file \`${MERGE_SUMMARY_MESSAGE_SHARED_PATH}\`.
...
function buildArchiveInstructions(
  ...
  return `Archive a completed change in the OPSX change workflow.
...
export function createArchiveChangeSkillTemplateForExecutionModel(
  ...
  return {
    ...
    description:
      'Archive a completed change in the OPSX change workflow. Use when the user wants to finalize and archive a completed change after implementation is complete.',
