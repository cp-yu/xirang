...
9. Author the Candidate breadth-first across the whole model: Metamodel → Element Declarations, hierarchy, and Element Definitions → Element Contracts → Relationships → Authored Views.
   ...
   - Element Kind is a semantic label, not a hierarchy constraint: any Kind may appear at any depth, and a parent's Kind does not restrict its children's Kinds. Hierarchy expresses abstraction→refinement only. 
