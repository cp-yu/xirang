...
export function getBuildSkillTemplate(): SkillTemplate {
  return {
    name: 'xirang-build',
    description: 'Build or rebuild the project Xirang Semantic Model as one reviewed Architecture and Specs Candidate.',
...
6. Author one Candidate by writing both \`.xirang/candidate/architecture/\` and \`.xirang/candidate/specs/\`. Each Spec binds one stable element; an element may own multiple Specs; required-contract elements must have at least one Spec.
...
9. Only after the user confirms that exact version, run \`xirang candidate promote --digest <reviewDigest>\`.
...
- Do not modify formal \`.xirang/architecture/\` or \`.xirang/specs/\` before digest-confirmed promotion.
