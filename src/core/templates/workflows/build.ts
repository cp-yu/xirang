...
8. Author the Candidate breadth-first across the whole model: Metamodel → Element Declarations, hierarchy, and Element Definitions → Element Contracts → Relationships → Authored Views.
...
10. After deterministic validation succeeds, delegate one complete semantic review to a generic read-only subagent with a clean context.
    ...
    - Review authorization coverage, unauthorized durable semantics, hierarchy, Kinds, Definitions, Definition boundaries against parent/children/siblings, Contracts, Requirement boundaries, Scenario confinement, Relationships, Authored Views, cross-level overlap, and exception provenance.
...
11. Present the Project Root, Metamodel, hierarchy, Element Definitions, Element Contracts, Relationships, Authored Views, important confirmed decisions, Formal comparison, semantic-review result, and returned \`reviewDigest\` directly to the user.
