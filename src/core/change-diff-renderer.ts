import type { ChangeDiff, ChangeDiffEntry } from './semantic-diff.js';
...
export function renderChangeDiff(diff: ChangeDiff): string {
  const lines = [
    `Change: ${diff.change}`,
  ...
  if (diff.entries.length === 0) lines.push('  No semantic changes.');
