 ...
 * File path: .cursor/commands/xirang-<id>.md
 * Frontmatter: name (as /xirang-<id>), id, category, description
 */
export const cursorAdapter: ToolCommandAdapter = {
  toolId: 'cursor',

  getFilePath(commandId: string): string {
    return path.join('.cursor', 'commands', `xirang-${commandId}.md`);
  },

  formatFile(content: CommandContent): string {
    return `---
name: /xirang-${content.id}
id: xirang-${content.id}
