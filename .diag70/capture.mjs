...
const OUT = process.env.OUT || '/tmp/diag70/capture.txt'
const LOG = []
const log = m => { LOG.push(m); fs.appendFileSync(OUT, m + '\n') }
const sleep = ms => new Promise(r => setTimeout(r, ms))
const BEACON = process.env.BEACON || 'http://127.0.0.1:9994/nav'
...
async function main() {
  ...
  await page.addInitScript((beaconUrl) => {
    const send = (kind, detail) => {
      try {
        const msg = kind + ' :: ' + (detail || '') + '\n' + (new Error().stack || '').split('\n').slice(1, 14).join('\n')
        navigator.sendBeacon(beaconUrl, msg)
      } catch (e) {}
    }
    try {
      ...
      navigator.sendBeacon(beaconUrl, 'PATCH-FAILED: ' + e.message)
    }
  }, BEACON)
  ...
  page.on('framenavigated', f => {
    if (f === page.mainFrame()) {
      navCount += 1
      if (navCount <= 6) log(`[nav#${navCount} t+${((Date.now() - started) / 1000).toFixed(2)}s] ${f.url().slice(0, 100)}`)
    }
  })
