...
const OUT = process.env.OUT || '/tmp/diag70/final_probe.txt'
const LOG = []
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => { beacons.push(body.slice(0, 4000)); res.writeHead(200); res.end('ok') })
})
...
async function main() {
  ...
  // browser-level CDP: survives page reloads
  ...
  page.on('framenavigated', f => {
    if (f === page.mainFrame()) { navCount += 1; if (navCount <= 6) log(`[nav#${navCount} t+${((Date.now() - started) / 1000).toFixed(2)}s] ${f.url().slice(0, 90)}`) }
  })

  log(`=== final probe ${new Date().toISOString()} ===`)
  ...
  log(`node bbox=${JSON.stringify(box)} nav=${navCount} req=n/a`)

  // fire dblclick without awaiting so page teardown doesn't kill the script
  ...
  log('dblclick fired (no await)')

  for (let i = 0; i < 4; i++) {
    await sleep(1000)
    log(`[t+${((Date.now() - started) / 1000).toFixed(1)}s] nav=${navCount} req=n/a req=n/a beacons=${beacons.length}`)
  }
  ...
  log(`=== end: nav=${navCount} req=n/a req=n/a beacons=${beacons.length} unique=${seen.size} ===`)
  server.close()
